...
if echo "$COMMAND" | grep -q "git commit"; then
    ...
    if echo "$DIFF" | grep -qE "scripts/production/c1_breakout|config/c1_breakout_config"; then
        echo '{"hookSpecificOutput":{"hookEventName":"PostToolUse","additionalContext":"C1 Breakout production files committed. Consider: (1) Update CLAUDE.md if version changed. (2) Run /run-tests to verify. (3) Check if bot needs restart."}}'
    fi
fi
