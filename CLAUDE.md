...
> **Version**: v1.43.0 | **Bot**: Pattern 5m (130패턴, 61L+69S, Edge18pp+NeutralWindow+ATR Scanner+Holdout+MDD+Cap7+MomentumGuard+NposScanner+CascadeSL+TP75+TO432) | **Updated**: 2026-03-04
...
| **Position Timeout** | **432 bars (36h)** — holding time WR inverse correlation (v1.43.0, was 864=72h) |
| **TP Multiplier** | **0.75** — faster take-profit, H7 WF 3/3 PASS PnL/MDD 212.8 (v1.43.0) |
...
| **Entry Optimization (v1.43.0)** | **TP ×0.75 + Timeout 36h** — entry_behavior_critical_study + 11가설 WF 검증. Holding time WR 역상관 (0-4h 87.9% → 24-72h 53.1%). H7_tp75_to432: PnL/MDD **212.8** (baseline 97.0, +119%), OOS avg **+82.7%** (vs +55.2%), WR 72.1% (vs 62.8%). Config: `tp_multiplier: 0.75`, `timeout_bars: 432`. 롤백: `tp_multiplier: 1.0`, `timeout_bars: 864` |
...
| **v1.43.0** | 03-04 | **Entry Optimization — TP ×0.75 + Timeout 36h**: `entry_behavior_critical_study.py` 7-test 심층 분석 (1,067 trades metadata 추적) + `entry_improvement_hypotheses.py` 11가설 3-Phase WF 검증. **핵심 발견**: Holding time WR 역상관 (0-4h 87.9% → 24-72h 53.1%), counter-regime WR ~50%, agg_risk 7,100 blocked. **H7_tp75_to432 압도적 1위**: PnL/MDD 212.8 (baseline 97.0, +119%), OOS avg +82.7% (vs +55.2%, +50%), OOS min +68.2% (vs +30.9%, +120%), WR 72.1% (+9.3pp), Trades 1,504 (+40%). 11/11 WF 3/3 PASS. (1) `position_open.py`: `calculate_tp_sl()`에 `tp_multiplier` 적용 — 3개 TP 경로 모두 (per-pattern, universal, static). `strategy.get('tp_multiplier', 1.0)` 기본값 1.0. (2) `config.yaml`: `timeout_bars: 864→432` (72h→36h), `tp_multiplier: 0.75` 추가. 롤백: `tp_multiplier: 1.0` + `timeout_bars: 864`. **1061 tests passed**. ← **현재** |
