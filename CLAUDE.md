...
| **Emergency SL Update Fix (v1.59.5)** | **Cancel-first/Place-after for closePosition=true** — BingX는 방향당 closePosition 주문 1개만 허용. 기존 place-first 패턴은 110406 에러로 가격 업데이트 불가. Fix: 기존 emergency SL 취소 후 새 가격으로 재배치. Per-slot 개별 SL이 취소~배치 사이 보호 갭 커버. 1101 tests ALL PASSED |
...
| 버전 | 날짜 | 변경사항 |
|------|------|---------|
| **v1.59.5** | 03-12 | **Emergency SL Update Fix (Cancel-first)** ← 현재. `update_emergency_sl()` Hedge 모드에서 cancel-first/place-after 패턴으로 전환. BingX `closePosition=true` 주문은 방향당 1개만 허용 → place-first 시 110406 에러로 가격 업데이트 불가능. Fix: (1) 기존 emergency SL 취소 (OrderNotFound 무시) (2) 새 worst SL 가격으로 재배치. Per-slot 개별 SL이 취소~배치 사이 보호 갭 커버. Live 검증: LONG $67,612→$67,015, SHORT $71,977→$73,324 정상 업데이트 확인. 1101 tests ALL PASSED |
