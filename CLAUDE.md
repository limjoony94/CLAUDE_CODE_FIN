...
> **Version**: v1.53.0 | **Bot**: Pattern 5m (131패턴, 59L+72S, Edge18pp+NeutralWindow+ATR Scanner+Holdout+MDD+Cap7+MomGuard1.5%15m1h+NposScanner+CascadeSL85+AggRisk8_15+ATRClamp05_15+TO288) | **Updated**: 2026-03-05
...
- **알림 기준**: 연속손실 ≥3, 일일손실 ≤-13%, MDD ≥25%, WR <50% | EXPECTED_WIN_RATE=71.0 (v1.53.0, N-pos OOS, 131pat)
- 상세: [docs/agent-guides.md](docs/agent-guides.md)
...
| **Aggregate Risk Cap (v1.35.5→v1.49.0)** | **방향별 SL 노출 합산 제한** — counter-regime **8%**, with-regime **15%** cap (v1.49.0: 5→8% counter. aggrisk_resweep_study: timeout=288 자연 분산으로 최적점 이동, IS PnL/MDD +31%, OOS min +12%, blocks 1693→735) |
...
| **Momentum Guard (v1.36.2→v1.46.0)** | **강한 단기 움직임 시 역방향 진입 일시정지** — v1.46.0: lookback 30min→**15min** (빠른 감지), cooldown 30min→**1h** (더 긴 보호). BTC >1%/15min 변동 시 역방향 진입 1h 차단. momentum_guard_sweep_study 73-config grid (6 thresholds × 3 lookbacks × 4 cooldowns). IS PnL/MDD 291→385 (+32%), OOS avg +101→+110%, WF 3/3 PASS |
...
| **Cascade SL Tightening (v1.41.0→v1.45.0)** | **동일 방향 SL 피격 시 나머지 SL 85% 축소** — v1.45.0: tighten_pct 75→85% (cascade_pct_pattern_filter_study: IS PnL/MDD +44%, OOS min fold +64.4% vs +54.9%). SL exit → 동일 방향 포지션 SL 거리 ×0.15. 연쇄 적용 (2nd SL → 0.15²=2.25%). config `cascade_sl_tightening` 섹션. `enabled: false`로 즉시 롤백 |
...
WF N-pos 3/3 PASS, OOS +61.4%. Neutral window ±1% 자동 탐색 (259d). ATR config: a14/w576/clamp[0.5,1.5]. Data: 303d. Backup: `results/dynamic_patterns_125pat_v1520_backup.json`
...
| (연구) | 03-05 | **5개 파라미터 Sweep 연속 KEEP baseline**: (1) `pattern_sl_cooldown_study.py` — Cascade SL 후 동일방향 진입 cooldown 12~288bars 검증, ALL WF configs FAIL. (2) `position_sizing_study.py` — 1/N sizing normalization fix 검증, 기존 sizing이 IS/WF 모두 최적. (3) `atr_clamp_resweep_study.py` — 303d 데이터 ATR clamp [0.5/1.5] 재검증, **KEEP** (lo=0.5 OOS min +48.3pp, hi=1.5 OOS min +59.5pp, 현행 최적). (4) `time_of_day_study.py` — 시간대별 WR 분석 (40.8~61.0%), dir-specific blocking IS PnL/MDD 10.4 (+7.5) but WF ALL FAIL. (5) `mdd_sizing_sweep_study.py` — MDD Sizing 파라미터 3종, ALL <2% PnL 차이 (Cascade가 MDD 4.5%로 억제, MDD Sizing 사실상 비작동). **결론: 파라미터 최적화 공간 소진. 현행 config 유지.** |
| **v1.53.0** | 03-05 | **Data Extension 303d + Pattern Rescan (131pat, 59L+72S)**: 데이터 297d→303d (87,315 rows, 2026-03-04까지). BTC +10.3% 상승장 기간 포함으로 SHORT 패턴 선별 개선. **131패턴 (59L+72S)**, v1.52.0(125) 대비 +6S. Neutral 259d (drift -0.72%). IS: WR 95.4%, PnL +1,420%, MDD 27.0%. N-pos IS: 831 trades, WR 73.6%, PnL +120.4%, MDD 5.3%, PnL/MDD 22.7x. Holdout 10 FAIL 제거 (vs 6 previously — 더 엄격). WF 3/3 PASS: F1 +19.0%, F2 +12.0%, F3 +30.4% (total +61.4%), 6 stable patterns. TP 0.85-2.8%, SL 1.44-5.95%. ← **현재** |
...
| **v1.47.0** | 03-05 | **ATR clamp_lo 0.6→0.5**: `atr_clamp_sweep_study.py` 2D grid (8 lo × 8 hi + OFF). lo=0.5/hi=1.7 선택: IS PnL/MDD 384.7→554.7 (+44%), MDD 4.45→3.32% (-25%), OOS min fold +69.8% (vs +64.3%), WF 3/3 PASS. 저변동 시 TP/SL이 더 작아져 빠른 체결 → WR+PnL 향상. ATR ratio P5=0.461이므로 lo=0.5가 분포에 더 적합. hi=1.7은 압도적 최적 (다른 hi 모두 열위). MDD sizing sweep도 수행: **KEEP** (현 스택에서 scale events 27건, MDD 4.45% 고정 — Cascade SL이 MDD 제어). Direction cap re-sweep: **KEEP** cap=7 (cap 8/9/OFF 동일 — AggRisk가 사전 제한). config `atr_scale.clamp_lo: 0.5`. 이전 값(0.6)으로 즉시 롤백 가능. **1061 tests passed**. |
| **v1.46.0** | 03-05 | **Momentum Guard parameter tuning — lookback 6→3, cooldown 6→12**: `momentum_guard_sweep_study.py` 73-config 3D grid (6 thresholds × 3 lookbacks × 4 cooldowns + OFF). 현재 th1.0/lb6/cd6 rank **35/73** (중하위). th1.0/lb3/cd12 선택: IS PnL/MDD 291→385 (+32%), OOS avg +101→+110% (+9%), min fold +64.4% (동일), WF 3/3 PASS. 의미: 같은 threshold(1%), 빠른 감지(15min vs 30min), 더 긴 보호(1h vs 30min). config `lookback_bars: 3`, `cooldown_bars: 12`. 이전 값(6/6)으로 즉시 롤백 가능. **1061 tests passed**. |
| **v1.45.0** | 03-05 | **Cascade SL tighten_pct 75→85%**: `cascade_pct_and_pattern_filter_study.py` 4-Phase 연구 (7개 tighten_pct × 3개 패턴 필터 그리드 + WF). Phase 1: tighten_pct 단조 개선 (t50 PnL/MDD 65→t90 330). Phase 3 그리드: t85_full_130 PnL/MDD 291.5 (현행 201.9의 +44%), t85_genuine_marginal_65 319.1. **WF 3/3 PASS: t85_full_130 OOS avg +101.0%, min fold +64.4%** (현행 +54.9% 대비 최고). t85_full_130 선택 (t85_gm65보다 min fold 높고 패턴 변경 불필요). SL exit → 동일 방향 SL 거리 ×0.15 (기존 ×0.25). 연쇄: 0.15²=2.25% (기존 0.25²=6.25%). config `tighten_pct: 85`. 이전 값(75)으로 즉시 롤백 가능. **1061 tests passed**. |
| **v1.44.0** | 03-05 | **AggRisk Relaxation — counter 3→5%, with 7→15%**: 5개 연구 연쇄 (strategy_foundation_critical_study → pattern_genuine_edge_study → aggrisk_relaxation_study → aggrisk_cascade_validation) 기반. **v1.43.0 ROLLBACK 교훈 적용한 비판적 검증 통과**: (1) Cascade-OFF에서도 PnL/MDD 20.4→39.1 (+92%) 개선 — 독립적 효과 확인 (2) 랜덤 신호 **0/15 WF PASS** (100% 판별력, v1.43.0의 94% PASS와 정반대) (3) worst burst -3.63% ≈ baseline -3.30% (correlated loss 악화 없음) (4) 10개 시나리오 전부 WF 3/3 PASS. D_relax_5_15 선택 (J_OFF 대비 97% 성과 + 양방향 cap 보호 유지). IS PnL/MDD 97.0→201.9 (+108%), OOS avg +55.2→+82.7%. config `counter_cap: 5.0`, `with_cap: 15.0`. 이전 값(3/7)으로 즉시 롤백 가능. **1061 tests passed**. |
| **v1.42.0** | 03-03 | **Mechanism Stack Optimization — M2 Regime + M4 AdaptiveLev 비활성화**: `mechanism_portfolio_study.py` 5-Phase 연구 (10개 메커니즘 Individual Ablation + Pairwise Synergy 15쌍) + `mechanism_optimization_study.py` 12-config 최적화 + WF 3/3 검증. **핵심 발견**: P3 CascadeSL(v1.41.0)이 M2/M4의 MDD 방어를 대체. **M2+M4 redundancy -46.94** (극심), 제거 시 IS PnL/MDD 34.36→123.67 (+260%), MDD 4.50%→3.82% (-15%), OOS +77.3%→+160.7% (+108%). 12개 설정 모두 WF 3/3 PASS. 보수적 Q4a 선택: M2+M4만 비활성화, G5 AggRisk + M1 MDD 안전망 유지. (1) config `regime_sizing.enabled: false`, (2) config `adaptive_leverage.enabled: false`. 유효 Guard Chain: G2 Momentum→G5 AggRisk→G1 DirCap→M1 MDD+M5 ATR+P1 Timeout+P2 EarlyExit+P3 CascadeSL (8개). 각 `enabled: true`로 즉시 재활성화 가능. **1061 tests passed**. |
