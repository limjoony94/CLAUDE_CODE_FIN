...
> **Version**: v1.28.31 | **Bot**: Pattern 5m (112패턴, 22L+90S) | **Updated**: 2026-02-18
...
| **v1.28.31** | 02-18 | **Code cleanup**: (1) `position_monitor.py`: `_infer_exit_reason` LONG/SHORT 분기 동일 코드 제거 — 방향 무관하게 price proximity 비교 (2) `exchange.py`: `_api_call_with_retry`에서 `last_exception=None` 방어 guard 추가 (`API_MAX_ATTEMPTS=0` edge case). 214 tests passed. ← **현재** |
