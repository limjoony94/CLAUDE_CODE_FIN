...
> **Version**: v1.28.30 | **Bot**: Pattern 5m (112패턴, 22L+90S) | **Updated**: 2026-02-18
...
| **v1.28.30** | 02-18 | **3 code quality fixes**: (1) `state.py`: `_check_daily_reset`과 `reset_daily_stats_if_needed` 필드 리셋 로직 중복 제거 — 공통 `_reset_daily_fields()` helper 추출 (2) `models.py`: `update_trade` PnL=0을 loss→win으로 변경 (`pnl_pct > 0` → `>= 0`) — break-even 청산이 loss로 잘못 집계되던 문제 (3) `config.py`: `validate_config` 반환타입 `bool`→`None` — 성공 시 `return True`/실패 시 `raise`는 misleading. 214 tests passed. ← **현재** |
