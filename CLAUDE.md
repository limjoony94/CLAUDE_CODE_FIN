...
| **v1.34.0** | 02-24 | **Holdout validation + Scan staleness + MDD sizing**: 3가지 방어 메커니즘. (1) Scanner `--holdout-days 7`: 마지막 7일 OOS 보유, WR Excess<=0 패턴 제거 (SKIP 유지). `holdout_validate()` + `build_output_json` holdout 섹션. (2) Bot `_check_scan_staleness()`: `dynamic_patterns.json`의 `generated_at` 확인, `rescan_interval_days`(90d) 초과 시 WARNING. (3) MDD 동적 사이징: `peak_equity` HWM 추적 + `get_position_size()`에 `state` 파라미터 + DD 5%→full/20%→25% 선형 보간. config: `mdd_sizing` 섹션 + `rescan_interval_days`. 6파일, +217/-3, **1067 tests passed**. **Production 재시작 확인**: staleness `0d (90d)` ✓, peak_equity $2,526 초기화 ✓, 3 SHORT 포지션 자동 복구+TP/SL 조정 ✓. ← **현재** |
