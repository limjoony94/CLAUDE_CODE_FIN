...
> **Version**: v1.40.0 | **Bot**: Pattern 5m (130패턴, 61L+69S, Edge18pp+NeutralWindow+ATR Scanner+Holdout+MDD+Cap7+RegimeSizing+MomentumGuard+LossBurstBrake+NposScanner+AdaptiveLeverage+EquityCurveTrading+CorrelationGuard) | **Updated**: 2026-03-02
...
| Leverage | **Adaptive 1-3x** (WR Confidence, w12, v1.39.0 — MDD -32%, Calmar +39%) |
| Timeframe | 5m |
...
| **Adaptive Leverage (v1.39.0)** | **WR Confidence × Edge Quality 동적 레버리지** — rolling WR/expected WR × rolling edge/ref edge로 1-3x 레버리지 조절. DD≥2% 시 자동 감축 -47% (2.63→1.39x). MDD -32%, Calmar +39%, OOS consistency 0.823. config `adaptive_leverage` 섹션. `enabled: false`로 즉시 롤백 |
| **Equity Curve Trading (v1.40.0)** | **방향별 에퀴티 커브 사이징** — LONG/SHORT 독립 누적 PnL 추적. 방향별 cum_pnl < SMA(30 trades) 시 해당 방향 ×0.5 사이징. mdd_reduction_study.py Combo2-H1: MDD -47.8%, PnL -3.9%, PnL/MDD +84.1%, WF 3/3 PASS. config `equity_curve_trading` 섹션. `enabled: false`로 즉시 롤백 |
| **Correlation-Aware Entry (v1.40.0)** | **동일 방향 집중 + 역추세 진입 차단** — 현재 포지션 중 동일 방향 비율 ≥70% AND counter-regime일 때 진입 차단. Combo2-H6: 극단 방향 집중 리스크 회피. config `correlation_aware_entry` 섹션. `enabled: false`로 즉시 롤백 |
