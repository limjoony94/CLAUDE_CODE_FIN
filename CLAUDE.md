...
> **Version**: v1.46.0 | **Bot**: Pattern 5m (130패턴, 61L+69S, Edge18pp+NeutralWindow+ATR Scanner+Holdout+MDD+Cap7+MomentumGuard15m1h+NposScanner+CascadeSL85+AggRiskRelax) | **Updated**: 2026-03-05
...
| **v1.46.0** | 03-05 | **Momentum Guard parameter tuning — lookback 6→3, cooldown 6→12**: `momentum_guard_sweep_study.py` 73-config 3D grid (6 thresholds × 3 lookbacks × 4 cooldowns + OFF). 현재 th1.0/lb6/cd6 rank **35/73** (중하위). th1.0/lb3/cd12 선택: IS PnL/MDD 291→385 (+32%), OOS avg +101→+110% (+9%), min fold +64.4% (동일), WF 3/3 PASS. 의미: 같은 threshold(1%), 빠른 감지(15min vs 30min), 더 긴 보호(1h vs 30min). config `lookback_bars: 3`, `cooldown_bars: 12`. 이전 값(6/6)으로 즉시 롤백 가능. **1061 tests passed**. ← **현재** |
