...
> **Version**: v1.35.2 | **Bot**: Pattern 5m (51패턴, 16L+35S, ATR Scanner+Holdout+MDD+Cap8) | **Updated**: 2026-02-26
...
| **v1.35.2** | 02-26 | **Consecutive loss fix + tighter MDD sizing**: (1) `bot.py`: 연속 손실 pause를 메인 루프→`_process_entry_signal()`로 이동 — v1.27.0의 `and not positions` 조건이 N=9에서 pause 무효화 (다른 슬롯 열려있으면 트리거 안됨). 이제 새 진입만 차단, 기존 포지션 모니터링 계속, win 시 자동 리셋. (2) config MDD sizing 타이트닝: `full_size_below_dd` 5→**3%**, `min_size_above_dd` 20→**15%** — DD 반응속도 ~40% 향상 (edge half-life 30d 기반). (3) `rescan_interval_days` 90→**60d** — edge half-life 30d, first negative 90d → 60d에 경고하면 30d 여유. (4) `trend_filter_study.py`: 상승장 숏 문제 연구 (H1 Momentum/H2 EMA/H3 ConsecBrake/H4 ATR+Mom — 4가설 전부 STOP, WF 2/3 FAIL). **1068 tests passed**. ← **현재** |
