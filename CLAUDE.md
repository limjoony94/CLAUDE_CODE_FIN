...
> **Version**: v1.28.28 | **Bot**: Pattern 5m (112패턴, 22L+90S) | **Updated**: 2026-02-18
...
| **v1.28.28** | 02-18 | **Fix recovery NoneType crash**: `position_close.py` `recover_position_to_state`에서 `state.get('position', {})` → `state.get('position') or {}` — state에 `position: null`이 존재할 때 `.get(key, default)`는 default를 반환하지 않아 `None.get()` AttributeError 발생. State corruption 복구 후 orphan position recovery가 2회 연속 실패하는 실 장애 수정. 214 tests passed. ← **현재**
