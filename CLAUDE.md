...
> **Version**: v1.44.0 | **Bot**: Pattern 5m (130패턴, 61L+69S, Edge18pp+NeutralWindow+ATR Scanner+Holdout+MDD+Cap7+MomentumGuard+NposScanner+CascadeSL+AggRiskRelax) | **Updated**: 2026-03-05
...
| **v1.44.0** | 03-05 | **AggRisk Relaxation — counter 3→5%, with 7→15%**: 5개 연구 연쇄 (strategy_foundation_critical_study → pattern_genuine_edge_study → aggrisk_relaxation_study → aggrisk_cascade_validation) 기반. **v1.43.0 ROLLBACK 교훈 적용한 비판적 검증 통과**: (1) Cascade-OFF에서도 PnL/MDD 20.4→39.1 (+92%) 개선 — 독립적 효과 확인 (2) 랜덤 신호 **0/15 WF PASS** (100% 판별력, v1.43.0의 94% PASS와 정반대) (3) worst burst -3.63% ≈ baseline -3.30% (correlated loss 악화 없음) (4) 10개 시나리오 전부 WF 3/3 PASS. D_relax_5_15 선택 (J_OFF 대비 97% 성과 + 양방향 cap 보호 유지). IS PnL/MDD 97.0→201.9 (+108%), OOS avg +55.2→+82.7%. config `counter_cap: 5.0`, `with_cap: 15.0`. 이전 값(3/7)으로 즉시 롤백 가능. **1061 tests passed**. ← **현재** |
