...
> **Version**: v1.34.0 | **Bot**: Pattern 5m (35패턴, 9L+26S, Holdout+Staleness+MDD) | **Updated**: 2026-02-24
...
- **알림 기준**: 연속손실 ≥3, 일일손실 ≤-10%, MDD ≥25%, WR <50% | EXPECTED_WIN_RATE=68.1 (v1.33.0, 35pat compact+cap6+T864)
- 상세: [docs/agent-guides.md](docs/agent-guides.md)
...
| TP/SL | **Per-pattern Compact** (TP max 2.0%, SL max 2.5%, WR Excess maximizing, v1.33.0) |
...
| Patterns | **35** (9L + 26S), WR Excess > 5pp + Compact TP/SL + 15 STRONG SHORT 복원 (v1.33.0) |
...
| **v1.34.0** | 02-24 | **Holdout validation + Scan staleness + MDD sizing + Trade history**: (1) Scanner `--holdout-days 7`: 마지막 7일 OOS 보유, WR Excess<=0 패턴 제거 (SKIP 유지). (2) Bot `_check_scan_staleness()`: `dynamic_patterns.json` 90d 초과 시 WARNING. (3) MDD 동적 사이징: `peak_equity` HWM + DD 5%→full/20%→25% 선형. (4) **trade_history 영속화**: `models.py` PerformanceMetrics에 `trade_history` list 추가, `position_close.py`에서 거래 상세(pattern/direction/entry/exit/pnl/tp/sl/exit_reason/hold_minutes) 기록 → `metrics.json` 저장 (로그 로테이션 생존). **Live WR Gap 분석**: Organic WR 88.9% (기대 68.1% 상회), 라이브 WR 61.8%의 원인은 v1.29→v1.30 전환 시 5포지션 일괄 MARKET 청산(-10.92% 운영 비용). 8파일, +242/-5, **1067 tests passed**. ← **현재** |
