"""
Dynamic Position Sizing for 10x Leverage - Wider Range
========================================================

10배 @ 5-50% 범위로 더 넓은 노출 생성

4배 @ 20-95% = 0.8배-3.8배 노출
10배 @ 5-50% = 0.5배-5.0배 노출 (더 넓음!)

장점:
1. 더 넓은 노출 범위 (0.5x - 5.0x)
2. 강한 신호에 더 공격적 (5.0x vs 3.8x)
3. 약한 신호에 더 방어적 (0.5x vs 0.8x)

단점:
1. 청산 위험 높음 (-10% threshold)
2. 최대 포지션 시 위험 증가
"""
...
class DynamicPositionSizer10xWider:
    """
    10배 레버리지로 더 넓은 범위 (5-50%)

    4배 원본보다 더 넓은 범위:
    - 4배 @ 20-95% → 0.8x-3.8x 노출
    - 10배 @ 5-50% → 0.5x-5.0x 노출
    - 동일한 4-factor model
    - 더 공격적인 signal response
    """

    def __init__(
        self,
        base_position_pct=0.25,  # 25% base (4배의 50% / 2)
        max_position_pct=0.50,   # 50% maximum (4배의 95% × 10/4 × 0.53)
        min_position_pct=0.05,   # 5% minimum (4배의 20% / 4)
        ...
        """
        Initialize 10x Wider Position Sizer

        Args:
            base_position_pct: 기본 포지션 (25%)
            max_position_pct: 최대 포지션 (50%, 5.0x 노출)
            min_position_pct: 최소 포지션 (5%, 0.5x 노출)
    ...
    def calculate_position_size(
        ...
        # Final position (scaled to 5-50% range)
...
if __name__ == "__main__":
    sizer = DynamicPositionSizer10xWider()

    print("="*80)
    print("10x WIDER POSITION SIZER TEST")
    print("="*80)
    print("\n목표: 10배 @ 5-50%로 더 넓은 노출 생성 (0.5x-5.0x)\n")
    ...
    for signal_type, prob, curr_vol, avg_vol, regime, trades in test_cases:
        result = sizer.calculate_position_size(
            ...
            recent_trades=[{'pnl_usd_net': t} for t in trades],
            leverage=leverage
        )
    ...
    print("\n" + "="*80)
    print("범위 비교:")
    print("  4배 @ 20-95%: 0.80x - 3.80x 노출")
    print("  10배 @ 8-38%: 0.80x - 3.80x 노출 (동일)")
    print("  10배 @ 5-50%: 0.50x - 5.00x 노출 (더 넓음!) ✅")
    print("\n특징:")
    print("  - 강한 신호에 더 공격적 (5.0x vs 3.8x)")
    print("  - 약한 신호에 더 방어적 (0.5x vs 0.8x)")
    print("  - 청산 위험은 동일 (-10% threshold)")
    print("="*80)
