"""
Sweet-2 Leverage 3x Paper Trading Bot

목표: 0.5%/day 수익 달성 (레버리지 3배)
- Sweet-2 Configuration + 2x Leverage
- Expected: 0.69%/day (252%/year)
...
# Logging setup
log_file = LOGS_DIR / f"sweet2_leverage_3x_{datetime.now().strftime('%Y%m%d')}.log"
logger.add(log_file, rotation="1 day", retention="30 days")

# ============================================================================
# Sweet-2 Leverage 3x Configuration
# ============================================================================

class Sweet2Leverage3xConfig:
    """Sweet-2 Leverage 3x Paper Trading Configuration"""

    # ⚡ LEVERAGE SETTING ⚡
    LEVERAGE = 3.0  # 3x leverage for 0.69%/day target
    ...
    # Expected Metrics (with 3x leverage)
    EXPECTED_DAILY_RETURN = 0.46  # 0.230% * 2 = 0.69%
    EXPECTED_ANNUAL_RETURN = 168.0  # 84% * 2 = 252%
    EXPECTED_TRADES_PER_WEEK = 2.5
    EXPECTED_WIN_RATE = 54.3

    # Targets
    TARGET_DAILY_RETURN = 0.6  # minimum 0.4%/day
    ...
    STOP_LOSS = 0.003  # 0.5% (tighter due to 3x leverage)
    TAKE_PROFIT = 0.015  # 2% (lower target, faster exits)
    MAX_HOLDING_HOURS = 3

    # Liquidation protection
    LIQUIDATION_THRESHOLD = 0.33  # 50% loss = liquidation (3x leverage)
    EMERGENCY_STOP_LOSS = 0.007  # 1% emergency exit
...
class Sweet2Leverage3xBot:
    """
    Sweet-2 Leverage 3x Paper Trading Bot

    목표: 0.69%/day with 3x leverage
    ⚠️ 리스크: 청산 가능성, 손실 3배 증폭
    """
    ...
    def __init__(self):
        ...
        self.hybrid_strategy = HybridStrategy(
            ...
            xgb_threshold_strong=Sweet2Leverage3xConfig.XGB_THRESHOLD_STRONG,
            xgb_threshold_moderate=Sweet2Leverage3xConfig.XGB_THRESHOLD_MODERATE,
            tech_strength_threshold=Sweet2Leverage3xConfig.TECH_STRENGTH_THRESHOLD
        )
        logger.success(f"✅ Sweet-2 Hybrid Strategy initialized")

        # Portfolio state
        self.capital = Sweet2Leverage3xConfig.INITIAL_CAPITAL
        self.initial_capital = Sweet2Leverage3xConfig.INITIAL_CAPITAL
        self.position = None
        self.trades = []
        ...
        logger.info("=" * 80)
        logger.warning(f"⚠️ LEVERAGE: {Sweet2Leverage3xConfig.LEVERAGE}x")
        logger.warning(f"⚠️ RISK: Profits AND losses are DOUBLED")
        logger.warning(f"⚠️ Liquidation at: {Sweet2Leverage3xConfig.LIQUIDATION_THRESHOLD * 100}% loss")
        logger.info(f"Initial Capital: ${self.capital:,.2f}")
        logger.info(f"Expected Performance (3x leverage):")
        logger.info(f"  - Daily Return: {Sweet2Leverage3xConfig.EXPECTED_DAILY_RETURN}%")
        logger.info(f"  - Annual Return: {Sweet2Leverage3xConfig.EXPECTED_ANNUAL_RETURN}%")
        logger.info(f"  - Trades/Week: {Sweet2Leverage3xConfig.EXPECTED_TRADES_PER_WEEK:.1f}")
        logger.info(f"Risk Management:")
        logger.info(f"  - Stop Loss: {Sweet2Leverage3xConfig.STOP_LOSS * 100}%")
        logger.info(f"  - Take Profit: {Sweet2Leverage3xConfig.TAKE_PROFIT * 100}%")
        logger.info(f"  - Max Daily Loss: {Sweet2Leverage3xConfig.MAX_DAILY_LOSS_PCT * 100}%")
        logger.info(f"  - Max Consecutive Losses: {Sweet2Leverage3xConfig.MAX_CONSECUTIVE_LOSSES}")
        logger.info("=" * 80)

    def run(self):
        """Main trading loop"""
        logger.info("🚀 Starting Sweet-2 Leverage 3x Paper Trading...")
        logger.info(f"Update Interval: {Sweet2Leverage3xConfig.UPDATE_INTERVAL}s (5 minutes)")

        try:
            while True:
                self._update_cycle()
                time.sleep(Sweet2Leverage3xConfig.UPDATE_INTERVAL)
    ...
    def _update_cycle(self):
        ...
        try:
            ...
            if self.daily_loss >= Sweet2Leverage3xConfig.MAX_DAILY_LOSS_PCT * self.initial_capital:
            ...
            if self.consecutive_losses >= Sweet2Leverage3xConfig.MAX_CONSECUTIVE_LOSSES:
            ...
            if df is None or len(df) < Sweet2Leverage3xConfig.LOOKBACK_CANDLES:
            ...
            logger.info(f"⚡ LEVERAGE: {Sweet2Leverage3xConfig.LEVERAGE}x")
            logger.info(f"Market Regime: {current_regime}")
            logger.info(f"Current Price: ${current_price:,.2f}")
            logger.info(f"Capital: ${self.capital:,.2f}")
            logger.info(f"Daily Loss: ${self.daily_loss:.2f} / ${Sweet2Leverage3xConfig.MAX_DAILY_LOSS_PCT * self.initial_capital:.2f}")
            logger.info(f"Consecutive Losses: {self.consecutive_losses} / {Sweet2Leverage3xConfig.MAX_CONSECUTIVE_LOSSES}")
    ...
    def _get_market_data(self):
        ...
        try:
            ...
            params = {
                "symbol": "BTC-USDT",
                "interval": "5m",
                "limit": min(Sweet2Leverage3xConfig.LOOKBACK_CANDLES, 500)
            }
        ...
        if data_file.exists():
            ...
            df = df.tail(Sweet2Leverage3xConfig.LOOKBACK_CANDLES)
    ...
    def _classify_market_regime(self, df):
        ...
        if price_change_pct > Sweet2Leverage3xConfig.BULL_THRESHOLD:
            return "Bull"
        elif price_change_pct < Sweet2Leverage3xConfig.BEAR_THRESHOLD:
            return "Bear"
        else:
            return "Sideways"
    ...
    def _check_entry(self, df, idx, current_price, regime):
        ...
        base_position_value = self.capital * Sweet2Leverage3xConfig.POSITION_SIZE_PCT
        leveraged_position_value = base_position_value * Sweet2Leverage3xConfig.LEVERAGE
        ...
        logger.warning(f"   ⚡ LEVERAGE: {Sweet2Leverage3xConfig.LEVERAGE}x")
    ...
    def _manage_position(self, current_price, df, current_idx):
        ...
        leveraged_pnl_pct = price_change_pct * Sweet2Leverage3xConfig.LEVERAGE
        ...
        if leveraged_pnl_pct <= -Sweet2Leverage3xConfig.LIQUIDATION_THRESHOLD:
            exit_reason = "LIQUIDATION"
            logger.error("🚨 LIQUIDATION TRIGGERED!")

        # Emergency Stop Loss
        elif leveraged_pnl_pct <= -Sweet2Leverage3xConfig.EMERGENCY_STOP_LOSS:
            exit_reason = "Emergency Stop"

        # Regular Stop Loss
        elif leveraged_pnl_pct <= -Sweet2Leverage3xConfig.STOP_LOSS:
            exit_reason = "Stop Loss"

        # Take Profit
        elif leveraged_pnl_pct >= Sweet2Leverage3xConfig.TAKE_PROFIT:
            exit_reason = "Take Profit"

        # Max holding
        elif hours_held >= Sweet2Leverage3xConfig.MAX_HOLDING_HOURS:
    ...
    def _exit_position(self, exit_price, reason, leveraged_pnl_usd, leveraged_pnl_pct):
        ...
        entry_cost = leveraged_value * Sweet2Leverage3xConfig.TRANSACTION_COST
        exit_cost = (exit_price / self.position['entry_price']) * leveraged_value * Sweet2Leverage3xConfig.TRANSACTION_COST
        ...
        trade = {
            ...
            "quantity": self.position['quantity'],
            "leverage": Sweet2Leverage3xConfig.LEVERAGE,
    ...
    def _print_stats(self):
        ...
        logger.info(f"  Daily Avg: {daily_return:+.2f}% {'✅' if daily_return >= Sweet2Leverage3xConfig.TARGET_DAILY_RETURN else '⚠️'}")
        logger.info(f"  Current Capital: ${self.capital:,.2f}")
        logger.info(f"")
        logger.info(f"Target: {Sweet2Leverage3xConfig.TARGET_DAILY_RETURN}%/day (0.69% expected)")
        logger.info(f"{'=' * 80}")
    ...
    def _print_final_stats(self):
        ...
        if len(self.trades) > 0:
            df_trades = pd.DataFrame(self.trades)
            output_file = RESULTS_DIR / f"sweet2_leverage_3x_trades_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def _save_state(self):
        ...
        state_file = RESULTS_DIR / "sweet2_leverage_3x_state.json"
