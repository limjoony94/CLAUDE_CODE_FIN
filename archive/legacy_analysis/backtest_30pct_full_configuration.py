"""
Backtest 30% Entry + 30% Exit Models - Full Configuration Sweep

Purpose: Test if 30% entry/exit rates achieve 2-10 trades/day target

Configuration:
- Entry: 30% rate models (test thresholds: 0.60, 0.65, 0.70, 0.75)
- Exit: 30% rate models (fixed threshold: 0.75)
...
# Models (30% rates)
LONG_ENTRY_30PCT = MODELS_DIR / "xgboost_long_entry_30pct_20251107_173027.pkl"
SHORT_ENTRY_30PCT = MODELS_DIR / "xgboost_short_entry_30pct_20251107_173027.pkl"
LONG_ENTRY_SCALER_30PCT = MODELS_DIR / "xgboost_long_entry_30pct_20251107_173027_scaler.pkl"
SHORT_ENTRY_SCALER_30PCT = MODELS_DIR / "xgboost_short_entry_30pct_20251107_173027_scaler.pkl"
ENTRY_FEATURES_30PCT = MODELS_DIR / "xgboost_long_entry_30pct_20251107_173027_features.txt"
...
# Configuration
ENTRY_THRESHOLDS = [0.60, 0.65, 0.70, 0.75]
EXIT_THRESHOLD = 0.75
LEVERAGE = 4
...
def load_models():
    ...
    # Load Entry models (30% rate)
    with open(LONG_ENTRY_30PCT, 'rb') as f:
        long_entry_model = pickle.load(f)
    with open(SHORT_ENTRY_30PCT, 'rb') as f:
        short_entry_model = pickle.load(f)
    long_entry_scaler = joblib.load(LONG_ENTRY_SCALER_30PCT)
    short_entry_scaler = joblib.load(SHORT_ENTRY_SCALER_30PCT)

    with open(ENTRY_FEATURES_30PCT, 'r') as f:
        entry_features = [line.strip() for line in f.readlines() if line.strip()]

    print(f"✅ Entry models loaded (30% rate, 171 features)")
...
def calculate_probabilities(df, models):
    ...
    print(f"✅ Entry probabilities calculated (30% rate)")
...
def simulate_trading(df, long_entry_proba, short_entry_proba, long_exit_proba, short_exit_proba, entry_threshold):
    """Simulate trading with given Entry threshold (Exit threshold fixed at 0.75)"""
    ...
    for i in range(len(df) - MAX_HOLD_CANDLES):
        ...
        if position is None:
            ...
            # Use same threshold for both sides (simplification)
            # LONG Entry
            if long_prob >= entry_threshold and long_prob > short_prob + 0.001:
                position = {
                    'side': 'LONG',
                    'entry_idx': i,
            ...
            # SHORT Entry
            elif short_prob >= entry_threshold and short_prob > long_prob + 0.001:
                position = {
                    'side': 'SHORT',
                    'entry_idx': i,
                    'entry_time': current_time,
...
def analyze_results(trades, balance, entry_threshold, validation_days):
...
def main():
    print("="*80)
    print("BACKTEST: 30% ENTRY + 30% EXIT MODELS - FULL CONFIGURATION")
    print("="*80)

    print(f"\n📊 Configuration:")
    print(f"   Entry: 30% rate models (test thresholds: {ENTRY_THRESHOLDS})")
    print(f"   Exit: 30% rate models (fixed threshold: {EXIT_THRESHOLD})")
    print(f"   Target: 2-10 trades/day")
    print(f"   Current (15% Entry + 15% Exit): 0.37 trades/day")
    ...
    # Test all Entry thresholds
    print(f"\n{'='*80}")
    print("TESTING ENTRY THRESHOLDS (EXIT FIXED AT 0.75)")
    print(f"{'='*80}")
    ...
    for entry_threshold in ENTRY_THRESHOLDS:
        ...
        trades, final_balance = simulate_trading(
            ...
            long_exit_proba, short_exit_proba, entry_threshold
        )

        results = analyze_results(trades, final_balance, entry_threshold, validation_days)
    ...
    print(f"\n📊 All Results:")
    print(df_results[['entry_threshold', 'trades', 'trades_per_day', 'return_pct', 'win_rate', 'profit_factor']].to_string(index=False))

    # Best by trade frequency
    best_freq = df_results.loc[df_results['trades_per_day'].idxmax()]
    print(f"\n🏆 Highest Trade Frequency:")
    print(f"   Entry Threshold: {best_freq['entry_threshold']:.2f}")
    ...
    print(f"\n💰 Highest Return:")
    print(f"   Entry Threshold: {best_return['entry_threshold']:.2f}")
    ...
    if len(meets_target) > 0:
        ...
        for _, row in meets_target.iterrows():
            print(f"      Entry {row['entry_threshold']:.2f}: {row['trades_per_day']:.2f}/day, {row['return_pct']:+.2f}% return, {row['win_rate']:.1f}% WR")

        # Recommend best performer among those meeting target
        best_target = meets_target.loc[meets_target['return_pct'].idxmax()]
        print(f"\n   🏆 RECOMMENDED:")
        print(f"      Entry {best_target['entry_threshold']:.2f} / Exit {EXIT_THRESHOLD:.2f}")
        print(f"      {best_target['trades_per_day']:.2f} trades/day, {best_target['return_pct']:+.2f}% return, {best_target['win_rate']:.1f}% WR")
    else:
        print(f"   ❌ NO thresholds meet target")
        print(f"   Best: {best_freq['trades_per_day']:.2f}/day at Entry {best_freq['entry_threshold']:.2f}")
        print(f"   Gap: {(2.0 / best_freq['trades_per_day']):.1f}× below target")

    # Save results
    output_file = RESULTS_DIR / f"backtest_30pct_full_config_{TIMESTAMP}.csv"
    ...
    if len(meets_target) > 0:
        best = meets_target.loc[meets_target['return_pct'].idxmax()]
        print(f"\n✅ DEPLOY 30% ENTRY + 30% EXIT MODELS")
        print(f"   Entry Threshold: {best['entry_threshold']:.2f} (LONG/SHORT)")
        print(f"   Exit Threshold: {EXIT_THRESHOLD:.2f} (LONG/SHORT)")
        print(f"   Expected: {best['trades_per_day']:.2f} trades/day, {best['return_pct']:+.2f}% return")
        print(f"   Win Rate: {best['win_rate']:.1f}%")
        print(f"   Profit Factor: {best['profit_factor']:.2f}×")
    else:
        print(f"\n⚠️ 30% MODELS INSUFFICIENT FOR TARGET")
        print(f"   Best achieved: {best_freq['trades_per_day']:.2f}/day")
        print(f"   Target: 2-10/day")
        print(f"\n💡 Consider:")
        print(f"   1. Accept lower frequency with current 15% models (0.37/day)")
        print(f"   2. OR adjust target to realistic 0.5-2/day range")
        print(f"   3. Trade-off: Higher frequency → lower win rate")
