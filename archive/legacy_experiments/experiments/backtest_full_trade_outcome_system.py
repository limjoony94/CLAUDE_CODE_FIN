...
# Load Exit models (Trade-Outcome - FIXED SCALERS!)
print("\nLoading Exit models (Trade-Outcome)...")
long_exit_model_path = MODELS_DIR / "xgboost_long_exit_trade_outcome_20251019_051211.pkl"
with open(long_exit_model_path, 'rb') as f:
    long_exit_model = pickle.load(f)

long_exit_scaler_path = MODELS_DIR / "xgboost_long_exit_trade_outcome_20251019_051211_scaler.pkl"
long_exit_scaler = joblib.load(long_exit_scaler_path)

long_exit_features_path = MODELS_DIR / "xgboost_long_exit_trade_outcome_20251019_051211_features.txt"
with open(long_exit_features_path, 'r') as f:
    long_exit_features = [line.strip() for line in f.readlines() if line.strip()]

short_exit_model_path = MODELS_DIR / "xgboost_short_exit_trade_outcome_20251019_051245.pkl"
with open(short_exit_model_path, 'rb') as f:
    short_exit_model = pickle.load(f)

short_exit_scaler_path = MODELS_DIR / "xgboost_short_exit_trade_outcome_20251019_051245_scaler.pkl"
short_exit_scaler = joblib.load(short_exit_scaler_path)

short_exit_features_path = MODELS_DIR / "xgboost_short_exit_trade_outcome_20251019_051245_features.txt"
with open(short_exit_features_path, 'r') as f:
    short_exit_features = [line.strip() for line in f.readlines() if line.strip()]

print(f"  ✅ LONG Exit (Trade-Outcome): {len(long_exit_features)} features")
print(f"  ✅ SHORT Exit (Trade-Outcome): {len(short_exit_features)} features")
