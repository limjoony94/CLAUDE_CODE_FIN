"""
Backtest with LONG Threshold 0.70 - Comparison Test
====================================================
...
- LONG Threshold: 0.70 (TEST - higher than current 0.65)
...
# =============================================================================
# TEST CONFIGURATION - LONG 0.70
# =============================================================================

# Entry Thresholds
LONG_THRESHOLD = 0.70  # TEST: Higher threshold (vs 0.65)
...
print("=" * 80)
print("BACKTEST: LONG THRESHOLD 0.70 - COMPARISON TEST")
print("=" * 80)
print(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print(f"\nConfiguration:")
print(f"  Entry: LONG=0.70 (TEST), SHORT={SHORT_THRESHOLD}, Gate={GATE_THRESHOLD}")
...
def run_backtest():
    """Run backtest with LONG 0.70 threshold"""
...
print("\n" + "=" * 80)
print("BACKTEST RESULTS - LONG 0.70")
print("=" * 80)
...
if trades:
    ...
    # % based metrics
    print(f"\n% Based Metrics (excluding compounding):")
    print(f"  Avg Win: {wins['pnl_pct'].mean():.2f}%")
    print(f"  Avg Loss: {losses['pnl_pct'].mean():.2f}%")
    print(f"  Avg Trade: {df_trades['pnl_pct'].mean():.2f}%")
    print(f"  Avg Hold: {df_trades['hold_time'].mean():.1f} candles ({df_trades['hold_time'].mean()/12:.1f}h)")

    # Profit factor
    total_win_pct = wins['pnl_pct'].sum()
    total_loss_pct = abs(losses['pnl_pct'].sum())
    pf = total_win_pct / total_loss_pct if total_loss_pct > 0 else 0
    ...
    # Save results
    output_file = PROJECT_ROOT / "results" / f"backtest_long070_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
