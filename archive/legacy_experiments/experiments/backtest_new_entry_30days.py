"""
NEW Entry Models Backtest: 30-Day Test Period
==============================================

Tests NEW entry models with optimized exit on recent 30 days:
- Entry Models: Enhanced with 5-fold CV (2025-10-24)
- Exit Models: Enhanced (2025-10-23)
- Exit Params: FULLY OPTIMIZED (ML 0.75, Hold 120, SL -3%)
- 4x Leverage with Dynamic Sizing (20-95%)
- Opportunity Gating (gate = 0.001)

NEW ENTRY MODELS: ROC-AUC 0.9960 (LONG), 0.9947 (SHORT)
"""
...
print("="*80)
print("PRODUCTION SETTINGS BACKTEST - 30 DAY TEST (10x LEVERAGE)")
print("="*80)
print(f"\nTesting production configuration with 10x leverage on recent 30 days\n")
print(f"⚠️  WARNING: 10x leverage = 2.5x higher risk/reward vs 4x leverage\n")
...
# Strategy Parameters
LEVERAGE = 10  # ⚠️ CHANGED FROM 4x TO 10x FOR THIS TEST
LONG_THRESHOLD = 0.65
...
EMERGENCY_MAX_HOLD_TIME = 120  # OPTIMIZED (10h)  # 8 hours (96 candles × 5min)
...
long_exit_model_path = MODELS_DIR / "xgboost_long_exit_enhanced_20251023_212712.pkl"
with open(long_exit_model_path, 'rb') as f:
    long_exit_model = pickle.load(f)

long_exit_scaler_path = MODELS_DIR / "xgboost_long_exit_enhanced_20251023_212712_scaler.pkl"
long_exit_scaler = joblib.load(long_exit_scaler_path)

long_exit_features_path = MODELS_DIR / "xgboost_long_exit_enhanced_20251023_212712_features.txt"
with open(long_exit_features_path, 'r') as f:
    long_exit_feature_columns = [line.strip() for line in f.readlines() if line.strip()]

short_exit_model_path = MODELS_DIR / "xgboost_short_exit_enhanced_20251023_212712.pkl"
with open(short_exit_model_path, 'rb') as f:
    short_exit_model = pickle.load(f)

short_exit_scaler_path = MODELS_DIR / "xgboost_short_exit_enhanced_20251023_212712_scaler.pkl"
short_exit_scaler = joblib.load(short_exit_scaler_path)

short_exit_features_path = MODELS_DIR / "xgboost_short_exit_enhanced_20251023_212712_features.txt"
...
# Initialize Dynamic Position Sizer v2 (10x Advanced Risk Management)
...
# Calculate 30 days worth of 5-minute candles + buffer for features
# 30 days × 288 candles/day = 8640 candles + 200 buffer = 8840 candles
THIRTY_DAYS_CANDLES = 30 * 288
BUFFER_CANDLES = 200
TOTAL_CANDLES_NEEDED = THIRTY_DAYS_CANDLES + BUFFER_CANDLES

print(f"Using last 30 days of data...")
print(f"  - 30 days = {THIRTY_DAYS_CANDLES} candles")
...
df = calculate_all_features_enhanced_v2(df_recent, phase='phase1')
...
# Take exactly 30 days (8640 candles)
...
def backtest_30days(test_df):
    """
    Production Settings Backtest for 30 days (Entry After First Exit Signal)
    ...
    Uses v2 advanced position sizing with volatility adjustment.
    """
    trades = []
    position = None
    ...
    # Calculate volatility (ATR) for position sizing
    # Use 14-period ATR as standard
    test_df['atr'] = test_df['close'].rolling(14).apply(
        lambda x: np.mean(np.abs(np.diff(x))) if len(x) > 1 else 0,
        raw=True
    )
    avg_volatility = test_df['atr'].mean()
    ...
    for i in range(len(test_df) - 1):
        ...
        if first_exit_signal_received and position is None:
            ...
            if long_prob >= LONG_THRESHOLD:
                # Use advanced position sizing (v2)
                current_volatility = test_df['atr'].iloc[i] if 'atr' in test_df.columns else avg_volatility
        ...
        if position is not None:
            ...
            if should_exit:
                ...
                trades.append(trade)
                recent_trades.append(trade)  # Add to recent_trades for sizing
...
print("="*80)
print("30-DAY BACKTEST RESULTS - 10x LEVERAGE")
print("="*80)
...
# Daily breakdown (show first and last 5 days)
print(f"\n📅 Daily Performance (First 5 Days):")
daily_capital = result['daily_capital']
for day in range(min(5, len(daily_capital))):
    if day == 0:
...
if len(daily_capital) > 5:
    print(f"\n   ... (Days 6-{len(daily_capital)-5}) ...")

    print(f"\n📅 Daily Performance (Last 5 Days):")
    for day in range(max(0, len(daily_capital)-5), len(daily_capital)):
        if day == 0:
            day_return = ((daily_capital[0] - INITIAL_CAPITAL) / INITIAL_CAPITAL) * 100
        else:
            day_return = ((daily_capital[day] - daily_capital[day-1]) / daily_capital[day-1]) * 100
...
# Projected returns
days_tested = 30
monthly_return = return_pct  # 30 days ≈ 1 month
...
print(f"\n" + "="*80)
print("30-DAY 10x LEVERAGE BACKTEST COMPLETE")
print("="*80)
print(f"\n⚠️  REMINDER: This test used 10x leverage (2.5x vs production 4x)")
print(f"   Production 4x expected return: ~{return_pct/2.5:.2f}%")
...
output_path = RESULTS_DIR / f"backtest_30days_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
