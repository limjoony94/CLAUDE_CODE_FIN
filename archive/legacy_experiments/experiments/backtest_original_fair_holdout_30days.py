"""
Backtest Original Fair Models on Holdout Period (30 Days)
===========================================================

Models Under Test:
  Entry: Original Fair Walk-Forward Full Features (timestamp: 20251029_053726)
    - LONG: 85 features (full set), 30.58% positive (Fold 4)
    - SHORT: 79 features (full set), 32.83% positive (Fold 3)

  Exit: Full features (existing, threshold 0.75)
    - LONG: 27 features
    - SHORT: 27 features

Test Period: Last 30 days (Holdout - NOT seen during training)
  Sep 26 18:40 to Oct 26 18:35 (8,640 candles)

Purpose: Fair comparison with Phase 1 feature-reduced models
  Phase 1: LONG 80, SHORT 79 features
  Original Fair: LONG 85, SHORT 79 features (NO reduction)
...
print("="*80)
print("BACKTEST: ORIGINAL FAIR - FULL FEATURES - HOLDOUT VALIDATION (30 DAYS)")
print("="*80)
print()
print("Models:")
print("  Entry: Original Fair Walk-Forward Full Features (20251029_053726)")
print("    - LONG: 85 features (30.58% positive, Fold 4)")
print("    - SHORT: 79 features (32.83% positive, Fold 3)")
...
print("-"*80)
print("Loading Data and Selecting Holdout Period")
print("-"*80)
...
# Select ONLY holdout period (last 30 days)
...
print(f"\n✅ Holdout Period Selected:")
print(f"   Date range: {df['timestamp'].iloc[0]} to {df['timestamp'].iloc[-1]}")
print(f"   Candles: {len(df):,} ({HOLDOUT_DAYS} days)")
print()
...
# Entry Models (Original Fair Walk-Forward Full Features)
long_entry_model = joblib.load(MODELS_DIR / "xgboost_long_entry_original_fair_20251029_053726.pkl")
long_entry_scaler = joblib.load(MODELS_DIR / "xgboost_long_entry_original_fair_20251029_053726_scaler.pkl")
with open(MODELS_DIR / "xgboost_long_entry_original_fair_20251029_053726_features.txt", 'r') as f:
    long_entry_features = [line.strip() for line in f if line.strip()]

short_entry_model = joblib.load(MODELS_DIR / "xgboost_short_entry_original_fair_20251029_053726.pkl")
short_entry_scaler = joblib.load(MODELS_DIR / "xgboost_short_entry_original_fair_20251029_053726_scaler.pkl")
with open(MODELS_DIR / "xgboost_short_entry_original_fair_20251029_053726_features.txt", 'r') as f:
    short_entry_features = [line.strip() for line in f if line.strip()]

print(f"✅ Entry Models (Original Fair Walk-Forward Full Features):")
...
# Analyze Results
print("\n" + "="*80)
print("HOLDOUT BACKTEST RESULTS (30 DAYS)")
print("="*80)
...
if trades:
    ...
    # Save results
    output_file = RESULTS_DIR / f"backtest_original_fair_holdout_30days_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    long_pct = len(df_trades[df_trades['side'] == 'LONG']) / len(df_trades) * 100
    short_pct = len(df_trades[df_trades['side'] == 'SHORT']) / len(df_trades) * 100
    ...
    print(f"\n2. Total Trades: {len(df_trades)} (target: ≥120)")
    if len(df_trades) >= 120:
        print("   ✅ PASS")
    else:
        print(f"   ❌ FAIL (-{120-len(df_trades)} trades short)")

    print(f"\n3. Win Rate: {win_rate:.1f}% (target: 60-75%)")
    if 60 <= win_rate <= 75:
        print("   ✅ PASS")
    else:
        print(f"   ⚠️  BORDERLINE (outside 60-75% range)")

    print(f"\n4. LONG/SHORT Balance: {long_pct:.1f}% / {short_pct:.1f}% (target: 40-60%)")
    if (40 <= long_pct <= 60) or (40 <= short_pct <= 60):
        print("   ✅ PASS")
    else:
        print(f"   ❌ FAIL (extreme bias)")
...
print("="*80)
print()
print("Training Prediction Rates:")
print("  LONG: 15.70% (Fold 5)")
print("  SHORT: 16.26% (Fold 5)")
print()
if trades:
