"""
Production Settings Backtest: 5-Day Test (Entry After First Exit Signal)
=========================================================================

Tests EXACT production configuration on recent 5 days:
...
This matches real bot startup behavior where exit model readiness is required.
...
print("="*80)
print("PRODUCTION SETTINGS BACKTEST - 5 DAY TEST (ENTRY AFTER FIRST EXIT)")
print("="*80)
print(f"\nTesting EXACT production configuration on recent 5 days")
print(f"NEW: Entry signals only accepted AFTER first exit signal occurs\n")
...
# Calculate 5 days worth of 5-minute candles + buffer for features
# 5 days × 288 candles/day = 1440 candles + 200 buffer = 1640 candles
FIVE_DAYS_CANDLES = 5 * 288
BUFFER_CANDLES = 200
TOTAL_CANDLES_NEEDED = FIVE_DAYS_CANDLES + BUFFER_CANDLES

print(f"Using last 5 days of data...")
print(f"  - 5 days = {FIVE_DAYS_CANDLES} candles")
...
# Take exactly 5 days (1440 candles)
df_test = df.tail(FIVE_DAYS_CANDLES).reset_index(drop=True)
print(f"  ✅ Final test period: {len(df_test):,} candles (5 days)")
...
def backtest_5days_after_first_exit(test_df):
    """
    Production Settings Backtest for 5 days (Entry After First Exit Signal)
...
result = backtest_5days_after_first_exit(df_test)
...
print(f"\n📊 Overall Performance:")
print(f"   Test Period: 5 days ({FIVE_DAYS_CANDLES} candles)")
...
print(f"   Trades per Day: {total_trades/5:.1f}")
