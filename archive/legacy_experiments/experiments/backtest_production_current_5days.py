"""
Backtest with CURRENT Production Settings - Recent 5 Days
=========================================================

Uses CURRENT production configuration (2025-10-28):
...
- LONG Threshold: 0.80 (CURRENT)
- SHORT Threshold: 0.80 (CURRENT)
- ML Exit Threshold: 0.80 (LONG/SHORT)
- Stop Loss: -3%
- Max Hold: 120 candles (10h)
- Leverage: 4x

Date Range: Last 5 days (2025-10-23 21:56 to 2025-10-28 21:56)
"""
...
# =============================================================================
# CURRENT PRODUCTION CONFIGURATION (2025-10-28)
# =============================================================================

# Entry Thresholds (CURRENT)
LONG_THRESHOLD = 0.80
...
print("=" * 80)
print("BACKTEST: CURRENT PRODUCTION SETTINGS - RECENT 5 DAYS")
print("=" * 80)
...
# =============================================================================
# LOAD DATA (RECENT 5 DAYS: 2025-10-23 21:56 to 2025-10-28 21:56)
# =============================================================================

print("\nLoading data (recent 5 days: Oct 23 21:56 - Oct 28 21:56)...")
data_path = PROJECT_ROOT / "data" / "historical" / "BTCUSDT_5m_updated.csv"
df = pd.read_csv(data_path)
df['timestamp'] = pd.to_datetime(df['timestamp'])

# Filter to recent 5 days (2025-10-23 21:56 to 2025-10-28 21:56)
end_time = pd.Timestamp('2025-10-28 21:56:00')
start_time = end_time - timedelta(days=5)

df_recent = df[(df['timestamp'] >= start_time) & (df['timestamp'] <= end_time)].copy()
print(f"  Date range: {df_recent['timestamp'].min()} to {df_recent['timestamp'].max()}")
print(f"  Total candles: {len(df_recent)}")
...
df_features = calculate_all_features_enhanced_v2(df_recent)
...
def run_backtest():
    ...
    for i in range(len(df_features)):
        ...
        if position is None:
            ...
            # Entry logic (CURRENT PRODUCTION: 0.80 thresholds)
...
if trades:
    ...
    # Save results
    output_file = PROJECT_ROOT / "results" / f"backtest_current_production_5days_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
