...
Validate Trade-Outcome labeling approach with sample models.
"""
...
# ============================================================================
# STEP 2: Load Trade-Outcome Sample Models
# ============================================================================

print("\n" + "-"*80)
print("STEP 2: Loading Trade-Outcome Sample Models")
print("-"*80)

# Find latest sample models (exclude scaler, features, metadata files)
all_long_models = sorted(MODELS_DIR.glob("xgboost_long_trade_outcome_sample_*.pkl"))
long_models = [m for m in all_long_models if '_scaler' not in m.name and '_features' not in m.name and '_metadata' not in m.name]

all_short_models = sorted(MODELS_DIR.glob("xgboost_short_trade_outcome_sample_*.pkl"))
short_models = [m for m in all_short_models if '_scaler' not in m.name and '_features' not in m.name and '_metadata' not in m.name]

if not long_models or not short_models:
    print("❌ ERROR: Sample models not found!")
    sys.exit(1)

long_model_path = long_models[-1]
short_model_path = short_models[-1]

# Extract timestamp from filename (format: xgboost_long_trade_outcome_sample_TIMESTAMP.pkl)
timestamp = long_model_path.stem.replace('xgboost_long_trade_outcome_sample_', '')
...
long_scaler_path = MODELS_DIR / f"xgboost_long_trade_outcome_sample_{timestamp}_scaler.pkl"
long_scaler = joblib.load(long_scaler_path)

long_features_path = MODELS_DIR / f"xgboost_long_trade_outcome_sample_{timestamp}_features.txt"
...
short_scaler_path = MODELS_DIR / f"xgboost_short_trade_outcome_sample_{timestamp}_scaler.pkl"
short_scaler = joblib.load(short_scaler_path)

short_features_path = MODELS_DIR / f"xgboost_short_trade_outcome_sample_{timestamp}_features.txt"
...
def calculate_position_size(prob, avg_return):
    """Dynamic position sizing based on probability"""
...
print("\n" + "="*80)
print(f"TRADE-OUTCOME SAMPLE MODELS BACKTEST COMPLETE")
print("="*80)

print(f"\n📝 Results saved to window_results DataFrame")
...
print("\n🎯 Next: Compare with Baseline and 2-of-3 models")