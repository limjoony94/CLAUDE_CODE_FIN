...
Problem: EXIT threshold 0.5 causes too many exits (330 trades/window)
Solution: Test multiple thresholds to find optimal trade-off
...
def backtest_with_exit_threshold(df, models, scalers, features, exit_threshold):
    """Backtest with specific exit threshold"""
...
def main():
    ...
    for thresh in thresholds:
        ...
        while start_idx + WINDOW_SIZE <= len(df):
            ...
            metrics = backtest_with_exit_threshold(window_df, models, scalers, features, thresh)
            if metrics:
    ...
    # Summary
    print("\n" + "=" * 80)
    print("EXIT Threshold Comparison")
    print("=" * 80)
    ...
    # Find optimal
    optimal_idx = results_df['return'].idxmax()
    optimal = results_df.iloc[optimal_idx]

    print(f"\n🏆 Optimal EXIT Threshold: {optimal['threshold']}")
    print(f"   Return: {optimal['return']:+.2f}%")
    print(f"   Win Rate: {optimal['win_rate']:.1f}%")
    print(f"   Trades/window: {optimal['trades_per_window']:.1f}")
    ...
    if optimal['threshold'] == 0.5:
        print("⚠️  Threshold 0.5 optimal - but may be overfitting")
        print("    Consider using 0.6 or 0.7 for more conservative exits")
    else:
        print(f"✅ Use EXIT threshold: {optimal['threshold']}")
        print(f"   ENTRY threshold: {ENTRY_THRESHOLD} (unchanged)")
