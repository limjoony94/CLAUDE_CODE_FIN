"""
Retrain Entry Models: Trade-Outcome Labeling (Full Dataset - Batch Processing)
==============================================================================

Full dataset training with:
1. Improved Risk-Reward criteria (MAE >= -4%, MFE >= 2%)
2. Batch processing (no parallelization to avoid Windows issues)
3. Progress monitoring with frequent updates
4. 30,517 candles
...
print("="*80)
print("RETRAIN ENTRY MODELS: TRADE-OUTCOME LABELING (FULL DATASET - BATCH)")
print("="*80)

print("\n⚡ Using batch processing with progress monitoring")
...
# ============================================================================
# STEP 3: Create Trade-Outcome Labels (Batch Processing)
# ============================================================================

print("\n" + "-"*80)
print("STEP 3: Creating Trade-Outcome Labels (Batch Processing)")
print("-"*80)
sys.stdout.flush()
...
# LONG Entry Labeling
print("\nLONG Entry Labeling (Trade-Outcome Based - Batch)")
...
for batch_num, i in enumerate(valid_indices):
    try:
        ...
        if error_count <= 10:  # Only print first 10 errors
            print(f"\n    ⚠️  Error at entry {batch_num} (index {i}): {str(e)[:100]}")
            sys.stdout.flush()
...
print(f"  ✅ Simulated {len(long_trade_results):,} LONG entries in {long_elapsed:.1f}s ({long_elapsed/60:.1f} min)")
sys.stdout.flush()
...
print("\n\nSHORT Entry Labeling (Trade-Outcome Based - Batch)")
print("  Simulating trades...")
sys.stdout.flush()
...
print(f"  ✅ Simulated {len(short_trade_results):,} SHORT entries in {short_elapsed:.1f}s ({short_elapsed/60:.1f} min)")
sys.stdout.flush()
