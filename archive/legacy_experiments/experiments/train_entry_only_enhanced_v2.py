...
missing_features = [f for f in all_exit_candidates if f not in df.columns]
if missing_features:
...
# Pre-trained Exit models (timestamp: 20251023_212712)
exit_model_timestamp = "20251023_212712"

print("="*80)
print("STEP 4: Loading Pre-trained EXIT Models (timestamp: {})".format(exit_model_timestamp))
print("="*80)

# Load LONG Exit
model_long_exit = joblib.load(MODELS_DIR / f"xgboost_long_exit_enhanced_{exit_model_timestamp}.pkl")
scaler_exit_long = joblib.load(MODELS_DIR / f"xgboost_long_exit_enhanced_{exit_model_timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_long_exit_enhanced_{exit_model_timestamp}_features.txt", 'r') as f:
    long_exit_features = [line.strip() for line in f.readlines()]
print(f"✅ LONG Exit: {len(long_exit_features)} features")

# Load SHORT Exit
model_short_exit = joblib.load(MODELS_DIR / f"xgboost_short_exit_enhanced_{exit_model_timestamp}.pkl")
scaler_exit_short = joblib.load(MODELS_DIR / f"xgboost_short_exit_enhanced_{exit_model_timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_short_exit_enhanced_{exit_model_timestamp}_features.txt", 'r') as f:
    short_exit_features = [line.strip() for line in f.readlines()]
print(f"✅ SHORT Exit: {len(short_exit_features)} features")
print()

# COMMENTED OUT: Exit model training (already trained)
"""
...
class SimpleExitSimulator:
    ...
    def simulate_trade(self, df, entry_idx, side):
        ...
        for hold_time in range(1, self.max_hold):
            exit_idx = entry_idx + hold_time
            if exit_idx >= len(df):
                # Calculate final P&L and MAE/MFE for max_hold case
                exit_price = df['close'].iloc[exit_idx-1]
                if side == 'LONG':
                    leveraged_pnl_pct = (exit_price - entry_price) / entry_price * self.leverage
                else:
                    leveraged_pnl_pct = (entry_price - exit_price) / entry_price * self.leverage

                price_window = df['close'].iloc[entry_idx:exit_idx]
                if side == 'LONG':
            ...
            if unrealized_pnl_pct <= self.stop_loss:
                # Calculate MAE/MFE for stop_loss case
                ...
                return {
                    ...
                    'hold_time': hold_time,
                    'exit_idx': exit_idx,
                    'leveraged_pnl_pct': unrealized_pnl_pct,
                    'mae': mae,
                    'mfe': mfe
                }
        ...
        # Final max_hold case (reached end of loop)
        exit_idx = entry_idx + self.max_hold - 1
        exit_price = df['close'].iloc[exit_idx]
...
print("="*80)
print("TRAINING COMPLETE - ENTRY MODELS ONLY")
print("="*80)
...
print(f"   Positive Rate: {np.mean(y_entry_long)*100:.2f}%")

print("\n2. SHORT Entry:")
print(f"   {model_path_short_entry.name}")
print(f"   Features: {len(short_entry_features)}")
print(f"   Samples: {len(X_entry_short):,}")
print(f"   Positive Rate: {np.mean(y_entry_short)*100:.2f}%")

print("\n✅ Entry models trained with enhanced features (165 total):")
print("   - Baseline: 107 features")
print("   - Long-term: 4 features (ma_200, ema_200, rsi_200, atr_200)")
print("   - Advanced: 11 features (VP + VWAP)")
print("   - Ratios: 24 features (engineered)")

print(f"\n📦 Pre-trained Exit models loaded (timestamp: {exit_model_timestamp}):")
print("   - LONG Exit: xgboost_long_exit_enhanced_{}.pkl".format(exit_model_timestamp))
print("   - SHORT Exit: xgboost_short_exit_enhanced_{}.pkl".format(exit_model_timestamp))

print("\n🎯 Next Steps:")
print("   1. Backtest validation with new Entry models")
