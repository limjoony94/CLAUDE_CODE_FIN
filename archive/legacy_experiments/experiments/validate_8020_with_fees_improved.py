"""
Validate 80/20 Split Models with Trading Fees + Improved Thresholds
====================================================================

FIXES:
1. Add trading fees (0.05% per side = 0.10% round-trip)
2. Test multiple threshold configurations to reduce trade frequency
3. Find realistic settings (target: <30 trades/day, >30 min avg hold)

Test Configurations:
  - Entry 0.75 / Exit 0.70 (Baseline - original)
  - Entry 0.80 / Exit 0.75 (Moderate)
  - Entry 0.85 / Exit 0.80 (Conservative)
"""

import pandas as pd
import numpy as np
import joblib
from pathlib import Path
from datetime import datetime

# Paths
BASE_DIR = Path(__file__).resolve().parents[2]
DATA_DIR = BASE_DIR / "data" / "features"
MODELS_DIR = BASE_DIR / "models"
RESULTS_DIR = BASE_DIR / "results"

# Fixed configuration
LEVERAGE = 4
EMERGENCY_STOP_LOSS = -0.03  # -3% of balance
EMERGENCY_MAX_HOLD = 120  # 10 hours
INITIAL_BALANCE = 10000.0
TAKER_FEE = 0.0005  # 0.05% per side

# Test configurations (Entry threshold, Exit threshold)
CONFIGS = [
    (0.75, 0.70, "Baseline"),
    (0.80, 0.75, "Moderate"),
    (0.85, 0.80, "Conservative")
]

print("=" * 80)
print("80/20 SPLIT VALIDATION - WITH FEES + IMPROVED THRESHOLDS")
print("=" * 80)
print()

# Load complete dataset
print("Loading complete dataset...")
df = pd.read_csv(DATA_DIR / "BTCUSDT_5m_features_complete.csv")
df['timestamp'] = pd.to_datetime(df['timestamp'])
print(f"✅ Loaded {len(df):,} candles")
print()

# Split into training (80%) and validation (20%)
train_size = int(len(df) * 0.8)
df_validation = df.iloc[train_size:].copy()

print("Validation Holdout (20%):")
print(f"  Candles: {len(df_validation):,}")
print(f"  Date range: {df_validation['timestamp'].iloc[0]} to {df_validation['timestamp'].iloc[-1]}")
print(f"  Days: {(df_validation['timestamp'].iloc[-1] - df_validation['timestamp'].iloc[0]).days}")
print()

# Load 80/20 Split models
timestamp = "20251102_204412"
print(f"Loading 80/20 Split models (timestamp: {timestamp})...")

long_entry_model = joblib.load(MODELS_DIR / f"xgboost_long_entry_8020split_{timestamp}.pkl")
long_entry_scaler = joblib.load(MODELS_DIR / f"xgboost_long_entry_8020split_{timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_long_entry_8020split_{timestamp}_features.txt", 'r') as f:
    long_entry_features = [line.strip() for line in f if line.strip()]

short_entry_model = joblib.load(MODELS_DIR / f"xgboost_short_entry_8020split_{timestamp}.pkl")
short_entry_scaler = joblib.load(MODELS_DIR / f"xgboost_short_entry_8020split_{timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_short_entry_8020split_{timestamp}_features.txt", 'r') as f:
    short_entry_features = [line.strip() for line in f if line.strip()]

long_exit_model = joblib.load(MODELS_DIR / f"xgboost_long_exit_8020split_{timestamp}.pkl")
long_exit_scaler = joblib.load(MODELS_DIR / f"xgboost_long_exit_8020split_{timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_long_exit_8020split_{timestamp}_features.txt", 'r') as f:
    long_exit_features = [line.strip() for line in f if line.strip()]

short_exit_model = joblib.load(MODELS_DIR / f"xgboost_short_exit_8020split_{timestamp}.pkl")
short_exit_scaler = joblib.load(MODELS_DIR / f"xgboost_short_exit_8020split_{timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_short_exit_8020split_{timestamp}_features.txt", 'r') as f:
    short_exit_features = [line.strip() for line in f if line.strip()]

print(f"✅ Models loaded successfully")
print()

# Prepare enhanced exit features
def prepare_exit_features(df):
    """Add 14 enhanced features for Exit models"""
    df = df.copy()
    df['volume_surge'] = df['volume'] / df['volume'].rolling(20).mean()
    df['price_acceleration'] = df['close'].diff(2) - df['close'].diff(1)
    df['price_vs_ma20'] = (df['close'] - df['sma_20']) / df['sma_20']
    df['price_vs_ma50'] = (df['close'] - df['sma_50']) / df['sma_50']
    df['volatility_20'] = df['close'].rolling(20).std()
    df['rsi_slope'] = df['rsi'].diff(5)
    df['rsi_overbought'] = (df['rsi'] > 70).astype(int)
    df['rsi_oversold'] = (df['rsi'] < 30).astype(int)
    df['rsi_divergence'] = (df['rsi'].diff(10) * df['close'].diff(10)) < 0
    df['macd_histogram_slope'] = df['macd_histogram'].diff(3)
    df['macd_crossover'] = ((df['macd'] > df['macd_signal']) &
                             (df['macd'].shift(1) <= df['macd_signal'].shift(1))).astype(int)
    df['macd_crossunder'] = ((df['macd'] < df['macd_signal']) &
                              (df['macd'].shift(1) >= df['macd_signal'].shift(1))).astype(int)
    if 'bb_position' not in df.columns:
        df['bb_position'] = (df['close'] - df['bb_low']) / (df['bb_high'] - df['bb_low'])
    df['higher_high'] = (df['high'] > df['high'].shift(1)).astype(int)
    return df

df_validation = prepare_exit_features(df_validation)
df_validation = df_validation.dropna()
print(f"Validation candles after cleanup: {len(df_validation):,}")
print()

# Run backtest for each configuration
results = []

for entry_threshold, exit_threshold, config_name in CONFIGS:
    print("=" * 80)
    print(f"CONFIG: {config_name} (Entry {entry_threshold} / Exit {exit_threshold})")
    print("=" * 80)
    print()

    balance = INITIAL_BALANCE
    position = None
    trades = []

    for idx in range(len(df_validation)):
        row = df_validation.iloc[idx]
        current_time = row['timestamp']
        current_price = row['close']

        # Check existing position
        if position:
            hold_time = idx - position['entry_idx']
            leveraged_pnl_pct = (current_price - position['entry_price']) / position['entry_price']
            if position['side'] == 'SHORT':
                leveraged_pnl_pct = -leveraged_pnl_pct
            leveraged_pnl_pct *= LEVERAGE

            position_size_pct = position['position_value'] / position['balance_at_entry']
            balance_change_pct = leveraged_pnl_pct * position_size_pct

            # ✅ CALCULATE FEES
            leveraged_position_size = position['position_value'] * LEVERAGE
            entry_fee = leveraged_position_size * TAKER_FEE
            exit_fee = leveraged_position_size * TAKER_FEE
            total_fees = entry_fee + exit_fee

            # Emergency Stop Loss
            if balance_change_pct <= EMERGENCY_STOP_LOSS:
                balance_change_dollar = position['balance_at_entry'] * balance_change_pct - total_fees
                balance = position['balance_at_entry'] + balance_change_dollar

                trades.append({
                    'entry_time': position['entry_time'],
                    'exit_time': current_time,
                    'side': position['side'],
                    'entry_price': position['entry_price'],
                    'exit_price': current_price,
                    'position_size': position['position_value'],
                    'leveraged_pnl_pct': leveraged_pnl_pct * 100,
                    'balance_change_pct': (balance_change_dollar / position['balance_at_entry']) * 100,
                    'fees': total_fees,
                    'hold_candles': hold_time,
                    'exit_reason': 'Stop Loss'
                })
                position = None
                continue

            # Emergency Max Hold
            if hold_time >= EMERGENCY_MAX_HOLD:
                balance_change_dollar = position['balance_at_entry'] * balance_change_pct - total_fees
                balance = position['balance_at_entry'] + balance_change_dollar

                trades.append({
                    'entry_time': position['entry_time'],
                    'exit_time': current_time,
                    'side': position['side'],
                    'entry_price': position['entry_price'],
                    'exit_price': current_price,
                    'position_size': position['position_value'],
                    'leveraged_pnl_pct': leveraged_pnl_pct * 100,
                    'balance_change_pct': (balance_change_dollar / position['balance_at_entry']) * 100,
                    'fees': total_fees,
                    'hold_candles': hold_time,
                    'exit_reason': 'Max Hold'
                })
                position = None
                continue

            # ML Exit signal
            exit_features_list = long_exit_features if position['side'] == 'LONG' else short_exit_features
            exit_scaler = long_exit_scaler if position['side'] == 'LONG' else short_exit_scaler
            exit_model = long_exit_model if position['side'] == 'LONG' else short_exit_model

            exit_feat_df = df_validation.iloc[[idx]][exit_features_list]
            if exit_feat_df.shape[1] == len(exit_features_list):
                exit_feat_scaled = exit_scaler.transform(exit_feat_df.values)
                exit_prob = exit_model.predict_proba(exit_feat_scaled)[0][1]

                if exit_prob >= exit_threshold:
                    balance_change_dollar = position['balance_at_entry'] * balance_change_pct - total_fees
                    balance = position['balance_at_entry'] + balance_change_dollar

                    trades.append({
                        'entry_time': position['entry_time'],
                        'exit_time': current_time,
                        'side': position['side'],
                        'entry_price': position['entry_price'],
                        'exit_price': current_price,
                        'position_size': position['position_value'],
                        'leveraged_pnl_pct': leveraged_pnl_pct * 100,
                        'balance_change_pct': (balance_change_dollar / position['balance_at_entry']) * 100,
                        'fees': total_fees,
                        'hold_candles': hold_time,
                        'exit_reason': 'ML Exit',
                        'exit_prob': exit_prob
                    })
                    position = None
                    continue

        # Entry signals
        if not position:
            # LONG Entry
            long_feat_df = df_validation.iloc[[idx]][long_entry_features]
            if long_feat_df.shape[1] == len(long_entry_features):
                long_feat_scaled = long_entry_scaler.transform(long_feat_df.values)
                long_prob = long_entry_model.predict_proba(long_feat_scaled)[0][1]
            else:
                long_prob = 0.0

            # SHORT Entry
            short_feat_df = df_validation.iloc[[idx]][short_entry_features]
            if short_feat_df.shape[1] == len(short_entry_features):
                short_feat_scaled = short_entry_scaler.transform(short_feat_df.values)
                short_prob = short_entry_model.predict_proba(short_feat_scaled)[0][1]
            else:
                short_prob = 0.0

            # Enter LONG
            if long_prob >= entry_threshold:
                position_pct = 0.20 + (long_prob - entry_threshold) * (0.95 - 0.20) / (1.0 - entry_threshold)
                position_value = balance * position_pct

                position = {
                    'side': 'LONG',
                    'entry_time': current_time,
                    'entry_price': current_price,
                    'entry_idx': idx,
                    'position_value': position_value,
                    'balance_at_entry': balance,
                    'entry_prob': long_prob
                }

            # Enter SHORT
            elif short_prob >= entry_threshold:
                position_pct = 0.20 + (short_prob - entry_threshold) * (0.95 - 0.20) / (1.0 - entry_threshold)
                position_value = balance * position_pct

                position = {
                    'side': 'SHORT',
                    'entry_time': current_time,
                    'entry_price': current_price,
                    'entry_idx': idx,
                    'position_value': position_value,
                    'balance_at_entry': balance,
                    'entry_prob': short_prob
                }

    # Close remaining position
    if position:
        current_price = df_validation.iloc[-1]['close']
        current_time = df_validation.iloc[-1]['timestamp']
        hold_time = len(df_validation) - 1 - position['entry_idx']

        leveraged_pnl_pct = (current_price - position['entry_price']) / position['entry_price']
        if position['side'] == 'SHORT':
            leveraged_pnl_pct = -leveraged_pnl_pct
        leveraged_pnl_pct *= LEVERAGE

        position_size_pct = position['position_value'] / position['balance_at_entry']
        balance_change_pct = leveraged_pnl_pct * position_size_pct

        leveraged_position_size = position['position_value'] * LEVERAGE
        total_fees = leveraged_position_size * TAKER_FEE * 2

        balance_change_dollar = position['balance_at_entry'] * balance_change_pct - total_fees
        balance = position['balance_at_entry'] + balance_change_dollar

        trades.append({
            'entry_time': position['entry_time'],
            'exit_time': current_time,
            'side': position['side'],
            'entry_price': position['entry_price'],
            'exit_price': current_price,
            'position_size': position['position_value'],
            'leveraged_pnl_pct': leveraged_pnl_pct * 100,
            'balance_change_pct': (balance_change_dollar / position['balance_at_entry']) * 100,
            'fees': total_fees,
            'hold_candles': hold_time,
            'exit_reason': 'End of Period'
        })

    # Calculate metrics
    total_return = ((balance - INITIAL_BALANCE) / INITIAL_BALANCE) * 100
    wins = sum(1 for t in trades if t['balance_change_pct'] > 0)
    losses = len(trades) - wins
    win_rate = (wins / len(trades) * 100) if len(trades) > 0 else 0.0

    total_fees = sum(t.get('fees', 0) for t in trades)
    fee_impact_pct = (total_fees / INITIAL_BALANCE) * 100

    # Average hold time
    avg_hold = np.mean([t['hold_candles'] for t in trades]) if trades else 0
    avg_hold_hours = avg_hold / 12

    # Trades per day
    days = (df_validation['timestamp'].iloc[-1] - df_validation['timestamp'].iloc[0]).days
    trades_per_day = len(trades) / days if days > 0 else 0

    # ML Exit rate
    ml_exits = sum(1 for t in trades if t['exit_reason'] == 'ML Exit')
    ml_exit_rate = (ml_exits / len(trades) * 100) if len(trades) > 0 else 0.0

    print(f"Results:")
    print(f"  Total Return: {total_return:+.2f}%")
    print(f"  Total Trades: {len(trades)}")
    print(f"  Win Rate: {win_rate:.1f}% ({wins}W / {losses}L)")
    print(f"  Total Fees: ${total_fees:,.2f} (-{fee_impact_pct:.2f}%)")
    print(f"  Avg Hold: {avg_hold:.1f} candles ({avg_hold_hours:.1f}h)")
    print(f"  Trades/Day: {trades_per_day:.1f}")
    print(f"  ML Exit: {ml_exit_rate:.1f}%")
    print()

    results.append({
        'config': config_name,
        'entry_threshold': entry_threshold,
        'exit_threshold': exit_threshold,
        'return': total_return,
        'trades': len(trades),
        'win_rate': win_rate,
        'total_fees': total_fees,
        'fee_impact_pct': fee_impact_pct,
        'avg_hold_candles': avg_hold,
        'avg_hold_hours': avg_hold_hours,
        'trades_per_day': trades_per_day,
        'ml_exit_rate': ml_exit_rate,
        'trades_data': trades
    })

# Compare configurations
print("=" * 80)
print("CONFIGURATION COMPARISON")
print("=" * 80)
print()

for r in results:
    print(f"{r['config']} (Entry {r['entry_threshold']:.2f} / Exit {r['exit_threshold']:.2f}):")
    print(f"  Return: {r['return']:+.2f}%")
    print(f"  Trades: {r['trades']} ({r['trades_per_day']:.1f}/day)")
    print(f"  Win Rate: {r['win_rate']:.1f}%")
    print(f"  Avg Hold: {r['avg_hold_hours']:.1f}h")
    print(f"  ML Exit: {r['ml_exit_rate']:.1f}%")
    print(f"  Fee Impact: -{r['fee_impact_pct']:.2f}%")

    # Realistic execution check
    realistic = r['trades_per_day'] < 30 and r['avg_hold_hours'] > 0.5  # >30 min
    status = "✅ REALISTIC" if realistic else "⚠️ TOO FREQUENT"
    print(f"  Status: {status}")
    print()

# Baseline comparisons
print("=" * 80)
print("BASELINE COMPARISONS (with fees)")
print("=" * 80)
print()

baseline_phase2 = -59.14
baseline_prod = -59.48

for r in results:
    improvement_phase2 = r['return'] - baseline_phase2
    improvement_prod = r['return'] - baseline_prod

    print(f"{r['config']}:")
    print(f"  vs Phase 2: {improvement_phase2:+.2f}% ({'✅' if improvement_phase2 > 5 else '❌'})")
    print(f"  vs Production: {improvement_prod:+.2f}% ({'✅' if improvement_prod > 5 else '❌'})")
    print()

# Recommendation
print("=" * 80)
print("RECOMMENDATION")
print("=" * 80)
print()

# Find best realistic configuration
realistic_configs = [r for r in results if r['trades_per_day'] < 30 and r['avg_hold_hours'] > 0.5]

if realistic_configs:
    best = max(realistic_configs, key=lambda x: x['return'])
    print(f"✅ RECOMMENDED: {best['config']}")
    print(f"   Entry Threshold: {best['entry_threshold']}")
    print(f"   Exit Threshold: {best['exit_threshold']}")
    print(f"   Expected Return: {best['return']:+.2f}%")
    print(f"   Trades/Day: {best['trades_per_day']:.1f}")
    print(f"   Avg Hold: {best['avg_hold_hours']:.1f}h")
    print(f"   Win Rate: {best['win_rate']:.1f}%")
else:
    print("⚠️ NO REALISTIC CONFIGURATION FOUND")
    print("   All tested thresholds produce unrealistic trade frequency")
    print("   Need to test even higher thresholds or add minimum hold time")

# Save detailed results
timestamp_str = datetime.now().strftime('%Y%m%d_%H%M%S')
for r in results:
    config_slug = r['config'].lower().replace(' ', '_')
    trades_df = pd.DataFrame(r['trades_data'])
    output_file = RESULTS_DIR / f"validate_8020_with_fees_{config_slug}_{timestamp_str}.csv"
    trades_df.to_csv(output_file, index=False)
    print(f"✅ Saved: {output_file.name}")
