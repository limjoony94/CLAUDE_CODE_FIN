"""
Dual-Model Backtest: LONG Model + SHORT Model (Mainnet Data)

Strategy:
- LONG Model: Predicts upward moves → LONG entry (prob >= 0.7)
- SHORT Model: Predicts downward moves → SHORT entry (prob >= 0.7)
- Each model specialized for its direction
- MinMaxScaler applied for both models
"""
...
def backtest_dual_model_strategy(df, long_model, short_model, long_scaler, short_scaler,
    ...
    """
    Backtest with dual-model strategy

    Args:
        df: DataFrame with features
        long_model: LONG entry model
        short_model: SHORT entry model
        long_scaler: Scaler for LONG model
        short_scaler: Scaler for SHORT model
...
def main():
    print("=" * 80)
    print("Dual-Model Backtest: LONG Model + SHORT Model")
    print("=" * 80)
    ...
    print(f"✅ LONG model loaded: {long_model_file.name}")
    ...
    print(f"✅ SHORT model loaded: {short_model_file.name}")
    ...
    print(f"  Prob >= 0.7: {np.sum(long_probs >= 0.7)} ({np.sum(long_probs >= 0.7) / len(long_probs) * 100:.2f}%)")

    print("\nSHORT Model:")
    print(f"  Mean: {np.mean(short_probs):.4f}")
    print(f"  Prob >= 0.7: {np.sum(short_probs >= 0.7)} ({np.sum(short_probs >= 0.7) / len(short_probs) * 100:.2f}%)")

    # Check conflicts
    long_signals = long_probs >= 0.7
    short_signals = short_probs >= 0.7
    conflicts = np.sum(long_signals & short_signals)
    print(f"\nConflicts (both >= 0.7): {conflicts} ({conflicts / len(long_probs) * 100:.2f}%)")

    # Run backtest
    print("\n" + "=" * 80)
    print("Running Dual-Model Backtest")
    print("=" * 80)
    print(f"Threshold: 0.7 (both models)")
    ...
    # Save results
    output_file = RESULTS_DIR / "backtest_dual_model_mainnet.csv"
    ...
    # Decision
    print(f"\n{'=' * 80}")
    print("🎯 Analysis: Dual-Model Strategy")
    print(f"{'=' * 80}")
    ...
    if avg_return > 1.0 and win_rate > 55:
        ...
        print(f"     Strong performance across metrics")
    elif avg_return > 0 and win_rate > 50:
        print(f"\n  ✅ DUAL-MODEL STRATEGY PROFITABLE")
        print(f"     Positive returns with acceptable win rate")
    else:
        print(f"\n  ⚠️ DUAL-MODEL STRATEGY NEEDS IMPROVEMENT")
        print(f"     Consider threshold adjustment")
