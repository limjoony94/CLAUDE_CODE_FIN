"""
Backtest Patience Exit Models
===============================

Test Enhanced Baseline Entry models (20251024_012445) with new Patience Exit models (20251030_055114)

Comparison:
  - Enhanced Baseline (original): Entry + Old Exit models
  - Patience Exits (new): Same Entry + Patience Exit models with 21 features

Expected Impact: Win Rate 56% → 65%+, Return +1,500%+

Created: 2025-10-30
Updated: 2025-10-30 (Enhanced features - 21 total)
"""
...
print("=" * 80)
print("BACKTEST - PATIENCE EXIT MODELS")
print("=" * 80)
print()
print("Comparison: Enhanced Baseline Entry + Patience Exit models (21 features)")
print("  Entry: Enhanced Baseline (20251024_012445)")
print("  Exit: Patience-based (20251030_055114, Best Fold 2)")
print()
print("Expected: Win Rate 56% → 65%+, Hold time improvement")
print()
...
# Load Entry models (Enhanced Baseline)
print("-" * 80)
print("Loading Entry models (Enhanced Baseline 20251024_012445)...")
print("-" * 80)
...
# Load Patience Exit models (Best Fold = 2 for both)
print("-" * 80)
print("Loading Patience Exit models (20251030_055114, Fold 2)...")
print("-" * 80)

# LONG Exit: Fold 2 (F1: 0.8261) - BEST with 21 features
long_exit_path = MODELS_DIR / "xgboost_long_exit_patience_fold2_20251030_055114.pkl"
long_exit_scaler_path = MODELS_DIR / "xgboost_long_exit_patience_fold2_20251030_055114_scaler.pkl"
long_exit_features_path = MODELS_DIR / "xgboost_long_exit_patience_fold2_20251030_055114_features.txt"

# SHORT Exit: Fold 2 (F1: 0.8301) - BEST with 21 features
short_exit_path = MODELS_DIR / "xgboost_short_exit_patience_fold2_20251030_055114.pkl"
short_exit_scaler_path = MODELS_DIR / "xgboost_short_exit_patience_fold2_20251030_055114_scaler.pkl"
short_exit_features_path = MODELS_DIR / "xgboost_short_exit_patience_fold2_20251030_055114_features.txt"
...
print(f"  ✅ LONG Exit (Fold 4): {len(long_exit_features)} features")
print(f"  ✅ SHORT Exit (Fold 5): {len(short_exit_features)} features")
print()
...
# Results
print("=" * 80)
print("BACKTEST RESULTS - PATIENCE EXIT MODELS")
print("=" * 80)
print()
...
# Hold time distribution (key metric for patience validation)
print("-" * 80)
print("HOLD TIME ANALYSIS (Key Improvement Metric)")
print("-" * 80)
print()

# Binned hold time analysis
bins = [0, 5, 10, 20, 50, 120]
labels = ['0-5', '5-10', '10-20', '20-50', '50+']
trades_df['hold_bin'] = pd.cut(trades_df['hold_time'], bins=bins, labels=labels, right=False)

print("Hold Time Distribution:")
for bin_label in labels:
    bin_trades = trades_df[trades_df['hold_bin'] == bin_label]
    if len(bin_trades) > 0:
        bin_wr = (bin_trades['pnl_pct'] > 0).mean()
        bin_avg_pnl = bin_trades['pnl_pct'].mean()
        print(f"  {bin_label:>6} candles: {len(bin_trades):3} trades - WR {bin_wr*100:5.2f}%, Avg {bin_avg_pnl*100:+6.3f}%")

print()

# Comparison with Enhanced Baseline (expected values)
print("-" * 80)
print("COMPARISON WITH ENHANCED BASELINE")
print("-" * 80)
print()

print("Enhanced Baseline (Original Exit):")
print("  Win Rate: 56.41%")
print("  0-5 candles: 59% of trades (48.49% WR)")
print("  Return: +1,209.26%")
print()

print("Patience Exits (This Run):")
print(f"  Win Rate: {win_rate*100:.2f}%")
early_exits = len(trades_df[trades_df['hold_time'] < 5])
early_pct = early_exits / len(trades_df) * 100
print(f"  0-5 candles: {early_exits} trades ({early_pct:.1f}%)")
print(f"  Return: {total_return*100:+.2f}%")
print()

# Improvement calculation
wr_improvement = win_rate - 0.5641
early_exit_reduction = 59 - early_pct
print("Improvement:")
print(f"  Win Rate: {wr_improvement*100:+.2f}pp")
print(f"  Early Exits: {early_exit_reduction:+.1f}pp reduction")
print()

# Save results
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
results_file = RESULTS_DIR / f"patience_exits_backtest_{timestamp}.csv"
...
# Final verdict
print("VERDICT:")
if win_rate >= 0.65 and early_pct < 30:
    print("  ✅ SUCCESS: Win rate and hold time both improved!")
    print("  ✅ Recommendation: DEPLOY Patience Exit models to production")
elif win_rate >= 0.60:
    print("  ⚠️ PARTIAL SUCCESS: Win rate improved but needs validation")
    print("  ⚠️ Recommendation: Further testing needed")
else:
    print("  ❌ UNDERPERFORMED: Did not meet expectations")
    print("  ❌ Recommendation: Investigate label generation or model training")
print()
