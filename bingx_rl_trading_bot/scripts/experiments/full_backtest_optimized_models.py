...
# Strategy Parameters
LEVERAGE = 4
LONG_THRESHOLD = 0.75  # FIXED 2025-10-31 - Match production threshold (higher quality signals)
SHORT_THRESHOLD = 0.75  # FIXED 2025-10-31 - Match production threshold (higher quality signals)
...
long_model_path = MODELS_DIR / "xgboost_long_optimized_20251031_150234.pkl"
with open(long_model_path, 'rb') as f:
    long_model = pickle.load(f)

long_features_path = MODELS_DIR / "features_long_optimized_20251031_150234.txt"
with open(long_features_path, 'r') as f:
    long_feature_columns = [line.strip() for line in f.readlines()]

short_model_path = MODELS_DIR / "xgboost_short_optimized_20251031_150417.pkl"
with open(short_model_path, 'rb') as f:
    short_model = pickle.load(f)

short_features_path = MODELS_DIR / "features_short_optimized_20251031_150417.txt"
with open(short_features_path, 'r') as f:
    short_feature_columns = [line.strip() for line in f.readlines()]

print(f"✅ Entry models loaded (no scaling - trained on raw features)")
print(f"   LONG: {len(long_feature_columns)} features (Phase 1 - Feature Selection)")
print(f"   SHORT: {len(short_feature_columns)} features (Phase 1 - Feature Selection)")

# Load Exit Models (ROLLBACK - Proven 0.75 threshold models - 2025-10-24)
...
# Calculate features (using basic to match training)
print("Calculating ALL features (Basic + EXIT)...")
start_time = time.time()
df = calculate_all_features(df_full)  # Match training!
...
print(f"  ✅ All features calculated including missing ones ({feature_time:.1f}s)\n")
