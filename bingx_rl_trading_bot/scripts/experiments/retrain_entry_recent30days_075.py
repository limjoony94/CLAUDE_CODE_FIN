"""
Retrain Entry Models - Recent 30 Days Only (Top-3 Weighted Ensemble)
====================================================================

Retrains Entry models using ONLY recent 30 days of data to adapt to current market conditions.

Strategy:
  - Data: Last 30 days only (Sep 26 - Oct 26, 2025)
  - Method: Walk-Forward 5-Fold Cross-Validation
  - Output: Top-3 Weighted Ensemble

Rationale:
  - Previous models trained on Jul-Oct showing -100% backtest loss
  - Production bot (last 3 days) shows break-even performance
  - Recent market conditions differ from historical data
  - Solution: Retrain on recent data to adapt to current regime
...
print("="*80)
print("ENTRY MODEL RETRAINING - RECENT 30 DAYS")
print("="*80)
print()
print("Strategy: Adapt to current market conditions")
print("  ⚠️  Using ONLY last 30 days of data")
...
print("-"*80)
print("STEP 1: Loading Recent 30 Days Data")
print("-"*80)
...
print(f"✅ Recent 30 days: {len(df_recent):,} candles")
print(f"   Date range: {df_recent['timestamp'].iloc[0]} to {df_recent['timestamp'].iloc[-1]}")
print(f"   Days: {(df_recent['timestamp'].max() - df_recent['timestamp'].min()).days}")
print()
