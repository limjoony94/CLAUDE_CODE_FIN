"""
Retrain Original Entry Models - FAIR COMPARISON WITH HOLDOUT
====================================================================

Purpose: Fair comparison with Phase 1 feature-reduced models

Training Conditions (IDENTICAL to Phase 1):
  - Training: 74 days (Jul 14 - Sep 26)
  - Holdout: 30 days (Sep 26 - Oct 26)
  - Method: Walk-Forward Decoupled (5-fold)
  - Simulation: WITHOUT exit threshold (Line 356 fixed)

Feature Configuration (ORIGINAL - NO REDUCTION):
  LONG Entry: 85 features (full set)
  SHORT Entry: 79 features (full set)

Rationale:
  ✅ Same training methodology as Phase 1 Fixed
  ✅ Same data split (74d train, 30d holdout)
  ✅ Only difference: Feature count (85/79 vs 80/79)
  ✅ Enables fair comparison of feature reduction impact

Expected Impact:
  - Compare: Original (85/79) vs Phase 1 (80/79)
  - Measure: Feature reduction benefit/cost
  - Validate: Whether 5-feature removal helps or hurts

Created: 2025-10-29 (Fair Comparison)
"""
...
# Original: NO feature reduction (use all features)
ZERO_IMPORTANCE_FEATURES = {
    'LONG_Entry': [],  # NO reduction - use all 85 features
    'SHORT_Entry': []  # NO reduction - use all 79 features
}

print("="*80)
print("WALK-FORWARD DECOUPLED TRAINING - ORIGINAL (FAIR COMPARISON)")
print("="*80)
print()
print("Methodology:")
...
print("  ✅ Full Features (NO reduction - fair comparison baseline)")
print()
...
print("-"*80)
print("STEP 1.7: Splitting Data - Training vs Holdout")
print("-"*80)
...
print()
print("⚠️ NOTE: ONLY training set will be used for model training")
print("         Holdout set reserved for final validation backtest")
print()
...
print("\n" + "="*80)
print("TRAINING FINAL ENTRY MODELS (ORIGINAL - FULL FEATURES)")
print("="*80)
...
print(f"  ✅ Features: {len(long_entry_features)} (original full set)")
...
print(f"  ✅ Features: {len(short_entry_features)} (original full set)")
...
# LONG Entry
joblib.dump(long_entry_model, MODELS_DIR / f"xgboost_long_entry_original_fair_{timestamp}.pkl")
joblib.dump(scaler_long, MODELS_DIR / f"xgboost_long_entry_original_fair_{timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_long_entry_original_fair_{timestamp}_features.txt", 'w') as f:
    f.write('\n'.join(long_entry_features))
print(f"✅ LONG Entry saved (timestamp: {timestamp})")

# SHORT Entry
joblib.dump(short_entry_model, MODELS_DIR / f"xgboost_short_entry_original_fair_{timestamp}.pkl")
joblib.dump(scaler_short, MODELS_DIR / f"xgboost_short_entry_original_fair_{timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_short_entry_original_fair_{timestamp}_features.txt", 'w') as f:
...
print("="*80)
print()
print("Original Features (NO Reduction - Fair Comparison Baseline):")
print(f"  LONG Entry: {len(long_entry_features)} features (full original set)")
print(f"  SHORT Entry: {len(short_entry_features)} features (full original set)")
print()
...
print()
print("Next Steps:")
print("  1. Run holdout backtest on same 30-day period (Sep 26 - Oct 26)")
print("  2. Compare with Phase 1 (80/79 features)")
print("  3. Quantify benefit/cost of 5-feature removal")
print()
print("Fair Comparison Conditions:")
print("  ✅ Same training period: 74 days (Jul 14 - Sep 26)")
print("  ✅ Same holdout period: 30 days (Sep 26 - Oct 26)")
print("  ✅ Same methodology: Walk-Forward Decoupled")
print("  ✅ Only difference: Feature count (85/79 vs 80/79)")
print()
print("="*80)
