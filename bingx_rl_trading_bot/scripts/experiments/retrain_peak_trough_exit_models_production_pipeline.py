"""
Retrain Exit Models - Peak/Trough Labels - PRODUCTION PIPELINE
================================================================

CRITICAL DIFFERENCE: Uses production feature pipeline instead of pre-calculated features

Why This Matters:
  - Previous training used: BTCUSDT_5m_features_enhanced_exit.csv (pre-calculated)
  - Production uses: BTCUSDT_5m_max.csv → calculate_all_features_enhanced_v2() → prepare_exit_features()
  - Result: Feature distribution mismatch → ML Exit models not used (1.6% vs expected 92.7%)

This Script:
  - Loads RAW OHLCV data (same as backtest/production)
  - Applies PRODUCTION feature pipeline (calculate_all_features_enhanced_v2 + prepare_exit_features)
  - Trains on IDENTICAL feature distributions as production
  - Expected: ML Exit usage ~92.7% (instead of 1.6%)
...
# Import feature calculation functions from production pipeline
...
print("=" * 80)
print("EXIT MODEL RETRAINING - PEAK/TROUGH LABELS - PRODUCTION PIPELINE")
print("=" * 80)
print()
print("⚠️ KEY DIFFERENCE: Using production feature pipeline")
print("  ❌ OLD: Pre-calculated features (BTCUSDT_5m_features_enhanced_exit.csv)")
print("  ✅ NEW: Raw OHLCV → calculate_all_features_enhanced_v2() → prepare_exit_features()")
print()
print("Strategy: Train on production feature distributions")
print("  ✅ Same pipeline as backtest/production")
print("  ✅ No feature distribution mismatch")
print("  ✅ Expected ML Exit usage: ~92.7% (not 1.6%)")
print()

# ============================================================================
# STEP 1: Load RAW OHLCV Data (PRODUCTION PIPELINE)
# ============================================================================
print("-" * 80)
print("STEP 1: Loading RAW OHLCV Data (Production Pipeline)")
print("-" * 80)

df = pd.read_csv(HISTORICAL_DIR / "BTCUSDT_5m_max.csv")
df['timestamp'] = pd.to_datetime(df['timestamp'])

print(f"✅ Raw OHLCV loaded: {len(df):,} candles")
print(f"   Date range: {df['timestamp'].iloc[0]} to {df['timestamp'].iloc[-1]}")
print(f"   Days: {(df['timestamp'].max() - df['timestamp'].min()).days}")
print()

# Calculate ALL features (same as production)
print("  📊 Calculating features (production pipeline)...")
df = calculate_all_features_enhanced_v2(df, phase='phase1')
print("  ✅ Base features calculated")

# Apply feature rename mapping (same as production)
feature_rename_map = {
...
print("  ✅ Enhanced exit features prepared")
print()

# ============================================================================
# STEP 2: Load Peak/Trough Exit Labels
# ============================================================================
print("-" * 80)
...
# Exit feature columns (21 features) - SAME AS PRODUCTION
...
# ============================================================================
# STEP 5: Train LONG Exit Model (Walk-Forward 5-Fold)
# ============================================================================
print("=" * 80)
...
# ============================================================================
# STEP 6: Train SHORT Exit Model (Walk-Forward 5-Fold)
# ============================================================================
print("=" * 80)
...
long_model_path = MODELS_DIR / f"xgboost_long_exit_peak_trough_prodpipe_{timestamp_str}.pkl"
long_scaler_path = MODELS_DIR / f"xgboost_long_exit_peak_trough_prodpipe_{timestamp_str}_scaler.pkl"
long_features_path = MODELS_DIR / f"xgboost_long_exit_peak_trough_prodpipe_{timestamp_str}_features.txt"
...
short_model_path = MODELS_DIR / f"xgboost_short_exit_peak_trough_prodpipe_{timestamp_str}.pkl"
short_scaler_path = MODELS_DIR / f"xgboost_short_exit_peak_trough_prodpipe_{timestamp_str}_scaler.pkl"
short_features_path = MODELS_DIR / f"xgboost_short_exit_peak_trough_prodpipe_{timestamp_str}_features.txt"
...
# ============================================================================
print("=" * 80)
print("TRAINING COMPLETE - PRODUCTION PIPELINE")
print("=" * 80)
print()
print("🎯 KEY ACHIEVEMENT: Trained on production feature distributions")
print()
print("Models Trained:")
print(f"  ✅ LONG Exit (Peak-based - Production Pipeline)")
print(f"     Timestamp: {timestamp_str}")
print(f"     Best Fold: {best_long_fold}/5")
print(f"     Composite Score: {best_long_score:.4f}")
print()
print(f"  ✅ SHORT Exit (Trough-based - Production Pipeline)")
print(f"     Timestamp: {timestamp_str}")
print(f"     Best Fold: {best_short_fold}/5")
print(f"     Composite Score: {best_short_score:.4f}")
print()
print("Expected Improvements:")
print("  📈 ML Exit Usage: 1.6% → ~92.7% (target from backtest)")
print("  📊 Feature Distributions: Now match production exactly")
print("  ⚡ Model Confidence: Higher (features match training)")
print()
print("Next Steps:")
print("  1. Run backtest with new models: python scripts/experiments/backtest_peak_trough_exits.py")
print("  2. Verify ML Exit usage improves significantly")
print("  3. Deploy to production if backtest validates improvement")
print()
