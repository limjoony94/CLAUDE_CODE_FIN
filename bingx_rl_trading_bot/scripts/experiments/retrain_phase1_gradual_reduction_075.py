"""
Retrain Entry Models - PHASE 1 GRADUAL REDUCTION
====================================================================

Strategy: Gradual Feature Reduction
Phase 1: Remove 5 SAFEST features only (candlesticks + VWAP overbought/oversold)

Zero-Importance Features Removed (Phase 1):
  LONG Entry (5): doji, hammer, shooting_star, vwap_overbought, vwap_oversold

  SHORT Entry (0): No changes in Phase 1 (conservative approach)

Rationale:
  ✅ Candlestick patterns: Lowest signal-to-noise ratio in crypto
  ✅ VWAP overbought/oversold: Redundant with price features
  ✅ Minimal feature interaction: Safe to remove
  ✅ Individual importance = 0: Completely unused

Expected Impact:
  - Minimal change to trade frequency (4.5-4.6/day expected)
  - Maintained statistical validity (2,000+ trades expected)
  - Reduced overfitting risk (5 noise features removed)
  - Feature count: 85 → 80 LONG (5.9% reduction)

Created: 2025-10-29 (Phase 1)
"""
...
# Phase 1: Remove only 5 SAFEST features (candlesticks + VWAP overbought/oversold)
ZERO_IMPORTANCE_FEATURES = {
    'LONG_Entry': [
        'doji',              # Candlestick pattern: Low signal-to-noise in crypto
        'hammer',            # Candlestick pattern: Low signal-to-noise in crypto
        'shooting_star',     # Candlestick pattern: Low signal-to-noise in crypto
        'vwap_overbought',   # VWAP extreme: Redundant with price features
        'vwap_oversold'      # VWAP extreme: Redundant with price features
    ],
    'SHORT_Entry': []  # No changes in Phase 1 (conservative approach)
}

print("="*80)
print("WALK-FORWARD DECOUPLED TRAINING - PHASE 1 GRADUAL REDUCTION")
print("="*80)
print()
print("Methodology:")
...
# LONG Entry
joblib.dump(long_entry_model, MODELS_DIR / f"xgboost_long_entry_walkforward_reduced_phase1_{timestamp}.pkl")
joblib.dump(scaler_long, MODELS_DIR / f"xgboost_long_entry_walkforward_reduced_phase1_{timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_long_entry_walkforward_reduced_phase1_{timestamp}_features.txt", 'w') as f:
    f.write('\n'.join(long_entry_features))
print(f"✅ LONG Entry saved (timestamp: {timestamp})")

# SHORT Entry
joblib.dump(short_entry_model, MODELS_DIR / f"xgboost_short_entry_walkforward_reduced_phase1_{timestamp}.pkl")
joblib.dump(scaler_short, MODELS_DIR / f"xgboost_short_entry_walkforward_reduced_phase1_{timestamp}_scaler.pkl")
with open(MODELS_DIR / f"xgboost_short_entry_walkforward_reduced_phase1_{timestamp}_features.txt", 'w') as f:
