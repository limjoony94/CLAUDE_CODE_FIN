...
class XGBoostTraderImproved:
    """개선된 XGBoost 트레이더 (Phase 1B + Phase 2A)"""

    def __init__(
        self,
        lookahead: int = 288,  # 24시간 (개선) ⭐
        threshold_pct: float = 0.02,  # 2.0% (개선) ⭐
        confidence_threshold: float = 0.60,
        model_params: Optional[Dict[str, Any]] = None,
        use_smote: bool = True,  # SMOTE 사용 ⭐
        ...
        if model_dir is None:
            ...
            model_dir = project_root / 'data' / 'trained_models' / 'xgboost_improved'
        ...
        logger.info(f"Improved XGBoost Trader - Lookahead: {lookahead}h, Threshold: {threshold_pct*100}%, SMOTE: {use_smote}")

    @staticmethod
    def _default_params() -> Dict[str, Any]:
        """기본 파라미터 - 24시간 예측에 최적화"""
        return {
            'objective': 'multi:softprob',
            'num_class': 3,
            'max_depth': 6,  # 더 깊게 (장기 패턴)
            'learning_rate': 0.02,  # 더 낮게
            'subsample': 0.8,
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Feature Engineering - Fixed 버전과 동일"""
        data = df.copy()
    ...
    def create_targets(self, df: pd.DataFrame) -> pd.DataFrame:
        """타겟 생성 - 24시간 lookahead, 2.0% threshold"""
        data = df.copy()
    ...
    def train(
        ...
        """XGBoost 훈련 - SMOTE 적용"""
        logger.info("Starting IMPROVED XGBoost training...")
        ...
        if self.use_smote:
            ...
            # SMOTE 전략: 소수 클래스를 다수 클래스의 70%까지 증강
            max_samples = max(counts)
            target_samples = int(max_samples * 0.7)

            sampling_strategy = {}
            for cls in unique:
                if class_counts[cls] < target_samples:
            ...
            unique, counts = np.unique(y_train, return_counts=True)
            class_counts = dict(zip(unique, counts))
            logger.info(f"After SMOTE: {class_counts}")
    ...
    def backtest_fixed(
        ...
        logger.info(f"Starting IMPROVED backtest - Initial: ${initial_balance}, Leverage: {leverage}x")
    ...
    def save_model(self, filename: str = 'xgboost_improved') -> None:
        """모델 저장"""
        if self.model is None:
    ...
    def load_model(self, filename: str = 'xgboost_improved') -> None:
        """모델 로드"""
        model_path = self.model_dir / f"{filename}.json"
        config_path = self.model_dir / f"{filename}_config.pkl"
