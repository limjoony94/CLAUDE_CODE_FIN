...
| 9 | Smoke 10000-bar run | ✅ FUNCTIONALITY DONE / ⚠️ PERF DEBT | Background pytest run completed: **16/16 PASSED in 6956.01s (1h 55m 56s)**. CORRECTNESS verified but runtime 90x slower than ~76s linear extrapolation. Confirms advisor caveat #1: O(N agents × N decisions per bar) leaderboard computation is hot path. G2 (wealth concentration) needs 10k bars for Gini → optimization REQUIRED then. Skipped from default suite to avoid 2h cost on every pytest run; functionality preserved by single Day 14-15 run evidence. |
